...
Techniques for discriminating fluorophores using their distinct spectral signatures or their intrinsic fluorescence lifetime do exist. However, existing fluorophore recovery methods are plagued by high errors due a number of factors: high overlap of fluorescence emission spectra; high photon counts required for fitting a complex decay model; and other fluorophores in the eye dominating the already weak return signal causing prohibitively high errors in the recovered fluorophore concentration. This motivates the need for a new\textbf{,} efficient\textbf{,} and robust unmixing method that addresses the shortcomings of \textbf{current unmixing techniques}: phasor analysis, spectral unmixing using matrix inversions, and unmixing directly using fluorescence lifetime.
\FloatBarrier
\section{Recovering Retinal Fluorophore Concentration Using a Multi-Exponential Fitting Model}\label{sec:lifeunmix}
The concentration of retinal fluorophores could be quantified using only measurements of the fluorescence lifetime in a single, broad, spectral band. Compared to sequentially scanning multiple spectral bands, a single-band fluorescence lifetime-base\textbf{d} method would reduce the influence of shot-noise in dim spectral bands as well as reducing image acquisition times. The process of fitting complex fluorescence decays does however require large photon fluxes. To model combinations of the 3 retinal biomarkers of interest – FAD, AGE, and A2E/Lipofuscin a bi-exponential fit is  a 6-exponential fit is needed to capture the bi-exponential decays of all 3 fluorophores.~\citeauthor{schweitzer2007towards} showed that each of these fluorophores exhibit bi-exponential decays and are shown in~\cref{tab:retlifetimes}\cite{schweitzer2007towards}.
\begin{table}
    \centering
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|}
        \hline
         &  Ground Truth & Deviation From Ground Truth &  Error $(\unit{\percent})$\\
         \hline
         $c_{FAD}$ & 0.4  & $+\num{3.1e-6}$ & \num{8e-4}\\
         $c_{AGE}$ &  0.2 & $+\num{1.8e-6}$ & \num{9e-4} \\
         $c_{A2E}$ & 0.4 & $-\num{6.3e-6}$ & \num{2e-4} \\
...
The above simulations were repeated with Poissonian noise applied to simulate shot noise for a range of photon fluxes: \numlist{e3;e5;e7;e12}, representing a range of photon fluxes typical \textbf{with} imaging the retina (\numrange{e3}{e5}), to idealised scenarios where the performance of future-generation SPAD arrays have improved significantly \textbf{(\numrange{e6}{e7})}. The effects of read-noise were not included in these simulations due to shot noise being the dominant noise source for photon fluxes larger than \num{e3}~\cite{henderson2019192}.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Results from recovering the abundance of retinal fluorophores from additive mixtures using their fluorescence lifetime with the inclusion of shot noise for where (\subref{subfig:retfitting1e3}),(\subref{subfig:retfitting1e5}),(\subref{subfig:retfitting1e7}),(\subref{subfig:retfitting1e12}) denote photon fluxes of \numlist{e3;e5;e7;e12} respectively. The recovered abundances (middle row) are plotted with the ground truths (top row) and the error in the unmixing corresponding to ~\cref{eq:recerror} is plotted (bottom row). Areas shaded white represent where the fluorophore concentration is 0 and so a relative error cannot be ascribed (\cref{eq:recerror}).}
\end{figure}
...
\end{figure}
At the lowest photon flux, the recovered abundances show no apparent correlation with the ground truth (\cref{subfig:retfitting1e3},\cref{subfig:retfitting1e5}) and only at the higher photon fluxes do these abundance maps resemble the ground truth (\cref{subfig:retfitting1e12}).\textbf{ In the regions with low fluorophore concentration,
the error in the unmixed abundances is only lower than \qty{50}{\percent} at prohibitively large photon fluxes of \num{e12}.} In the current literature, a photon flux of \numrange{e3}{e4} is achievable for \textit{in-vivo} retinal FLIM imaging for practical integrations times of $\approx\qty{60}{\second}$. Using this fitting-based technique an integration time in excess of 2 millennia would be required for this technique to be suitable. \textbf{These results also demonstrate the large photon fluxes required for fitting more complex decay models: a single exponential decay requires at least 1000 photons: a bi-exponential decay requires \numrange{e4}{e5}~\cite{kollner1992many}; and for a tri-exponential decay it has been reported that at least \numrange{5e5}{e6} photons are required\cite{klemm2015flimx}.}
...
In the literature, the phasor-SFLIM technique was reported to robustly unmix fluorophores at low photon fluxes~\cite{scipioni2021phasor,scipioni2022phasor}  – for these 3 retinal fluorophores this could not be replicated. In the above simulation the unmixing method showed poor performance even without the inclusion of photon shot noise and would miscategorise the retinal fluorophores in the recovered spectra. This poor performance is most likely due to the high spectral overlap and similar decay profiles of FAD, AGE, and A2E which in phasor space gives endmembers that are not well separated and in the spectral phasor plot no prominent vertices are found. While this analysis did not attempt to model spectra contamination due to absorption in blood and vasculature or the effects the lens fluorescence it can be concluded that the phasor-SFLIM algorithm is not suitable for measuring concentrations of the retinal FAD and was not pursued further. Where phasor-SFLIM does perform well is for spectral-fluorescence lifetime imaging of stained samples where fluorophores can be chosen to minimise spectral overlap and exhibit differing fluorescence lifetimes. 
The methods for unmixing fluorophores using fluorescence lifetimes, and spectrally-resolved fluorescence lifetimes have been unable to robustly quantify retinal FAD concentrations at photon-fluxes that are commensurate with \textit{in-vivo} FLIM imaging of the human retina. This motivates the need for a new SFLIM unmixing method which will be explored in~\cref{chap:tensSFLIM}. However, it would be prudent to investigate the performance of other state-of-the-art unmixing methods that do not utilise fluorescence lifetimes and instead would discriminate fluorophores using subtle variations in the fluorescence emission spectra. In the next section, the capabilities of spectral unmixing for unmixing retinal fluorophores can then be used as a benchmark for these lifetime based unmixing methods and the new SFLIM unmixing method proposed in~\cref{chap:tensSFLIM}.
\FloatBarrier
\section{Spectral Unmixing Using Matrix Inversions}\label{sec:specunmix}
In remote sensing, the endmembers\textbf{, or unique species,} are discriminated using variations in spectral signatures to separate buildings, from foliage\textbf{,} from rivers in the scene where generally\textbf{,} buildings will often appear spectrally flat, foliage will peak in the green\textbf{,} and rivers and water features will peak in the blue spectral regions. In these applications the endmembers are unmixed by assuming the following linear mixing model
\begin{align}
...
\begin{align}
    ...
    & \mathbf{a} &= \begin{bmatrix} a_{0} \\ a_{1} \\ \vdots \\ a_{n}\end{bmatrix}, \quad\text{and}\\
    \\
     \quad \mathbf{M} &= \begin{bmatrix} M(\lambda_{0}) & M(\lambda_{1}) & \cdots & M(\lambda_{j}) \end{bmatrix}
\end{align}
The recovered endmember abundances, $\bar{\mathbf{a}}$ are then found using a simple matrix inversion:
\begin{align}
    \bar{\mathbf{a}} &= \mathbf{S}^{-1}\mathbf{M}, \label{eq:matinv}
\end{align}
For over-determined systems - where the number of spectral measurements is greater than the number of unknown endmember abundances  - the matrix $\mathbf{S}$ is non-square and so the inverse $\mathbf{S}^{-1}$ doesn't exist. The Moore-Penrose Pseudo Inverse, denoted as $\mathbf{S}^{+}$:
\begin{align}
    \mathbf{S}^{+} &= (\mathbf{S}^{T}\mathbf{S})^{-1}\mathbf{S}^{T}, \label{eq:mpinvmatrix}
\end{align}
 It also extends some useful properties of the traditional matrix inverse:
\begin{align}
    \mathbf{A}^{-1} &: \mathbf{A}^{-1}\mathbf{A} = \mathbb{I} \implies \mathbf{A}^{+}: \mathbf{A}\mathbf{A}^{+}\mathbf{A} = \mathbf{A} \label{eq:mpinvidentity}
\end{align}
The matrix condition number, $\kappa$,
...
\end{equation}
is a convenient metric for assessing how sensitive the inversion process is to noise given a set of the endmember spectra that are being unmixed, the number of detection bands as well as the location of these bands and their width. In the inversion process, a noise in the measurement $\mathbf{M}$ of $\nicefrac{\delta \mathbf{M}}{\mathbf{M}}$ will cause an error in the unmixed abundance, $\mathbf{a}$, on the order of $\nicefrac{\delta \mathbf{a}}{\mathbf{a}}$ - scaled by the condition number, $\kappa(\mathbf{S})$:
\begin{equation}
...
\end{equation}
\textbf{The loss in accuracy in $\mathbf{a}$ due to some noise in $\delta \mathbf{M}$ scales with the condition number such that in general, $k \propto \log_{10}(\kappa(\mathbf{S}))$ digits of accuracy are lost~\cite{cheney2012numerical}.} For recovering the abundances of retinal fluorophores, which will vary between  \numlist{0; 1}, a condition number of $\kappa(\mathbf{S}) < \num{e2}$ is required to have a desired error of less than \qty{10}{\percent} to enable more subtle changes in fluorophore concentrations to be confidently measured. However, simply measuring the presence of a retinal FAD would be sufficient to show SFLIM could be a viable technique for longitudinal monitoring of retina health.
\FloatBarrier
\subsection{Optimisation of Spectral Detection Bands for\\ Recovering Concentrations of Retinal Fluorophores}
The condition number was used as a metric for finding the optimal location and number of detection bands \textbf{where the spectral images are captured sequentially}. While narrow, contiguous bands do give higher spectral resolution each band exhibits lower SNR or increased $\nicefrac{\delta \mathbf{M}}{\mathbf{M}}$ and longer image acquisition times. For this proposed spectral-imaging scheme, a system of \num{6} bands, shown in \cref{fig:retspectrabands}, was found using an exhaustive search that minimised the condition number $< 10$ of all 40 contiguous filters with width \qty{5}{\nm} spanning a wavelength range of \qtyrange{500}{700}{\nm} giving $\binom{40}{6} \approxeq \num{3.8e6}$ possible groups of 6:
\begin{equation}\label{eq:nchoosek}
    \binom{n}{m} = \frac{n!}{k!(n-k)!},
\end{equation}
where $n = 40$ is the total number of unique filter locations, and $k = 6$ is the desired number of filters. The search gave an optimal combination of filters with cut-on wavelengths of \qtylist{500;535;630;640;655;660}{\nm} and a condition number of $\kappa = \num{8.101}$ indicating an at most one digit of precision will be lost. Surprisingly the optimisation gave a set of filters that are located primarily where AGE has the contribution to the fluorescence signal although this in the wavelength range  where AGE and FAD appear equally but may continually cross each other. These subtle variations in spectra could be leveraged for improved unmixing capabilities but would rely on low-noise to be confident that small variations in the measured spectra are due to spectral variation in the AGE and FAD. With the benefit of hindsight, using wider spectral bands, \textbf{using fewer but wider spectral bands,} or using the specifications of commercially available bandpass filters would have given a more accurate assessment of the capabilities of spectral unmixing for quantifying retinal FAD.
However, for the purposes of providing a benchmark for the SFLIM unmixing method proposed in \cref{chap:tensSFLIM} this is sufficient, in that an optimised SFLIM unmixing method should be as accurate as a sub-optimal spectral umixing method.
\begin{figure}
    \centering
...
\end{figure}
The performance of this detection scheme was then quantified using the previously utilised abundance maps for both noiseless data and data with shot-noise corresponding to the detection of \num{e5} photons over all 6 spectral bands by calculating the RMS errors:
\begin{equation}\label{eq:RMSerror}
    \mathbf{\epsilon}_{RMS} = \sqrt{\frac{\sum_{n \geq 1}^{N}\mathbf{\epsilon}^{2}}{N}},
\end{equation}
where $N$ is the number of samples, and $\epsilon$ is the difference between the ground truth abundance and the recovered abundance. \textbf{For noiseless data, the RMS error, $\epsilon$, is on the order of \qty{e-12}{\percent} (see \cref{fig:spectunmixoptbands}a) however when shot noise, shown in \cref{fig:spectunmixoptbands}b, is applied the average error in the recovered abundance of FAD rises to \qty{91.8}{\percent} for low concentrations.}
\begin{figure}[htb]
    \begin{subfigure}[b]{\textwidth}
        \centering
...
\end{figure}
For large relative concentrations of FAD - and for the other retinal fluorophores - the RMS error is low implying that if FAD is the dominant fluorophore in \textbf{the} retina then spectral unmixing could be capable of detecting changes in FAD commensurate with those expected from changes in metabolic health. Since the retinal fluorophores, AGE, FAD, and A2E have not been quantitatively measured then this cannot be relied on.

\FloatBarrier
\subsection{Appraising the Feasibility of Using Spectral Imaging for Measuring FAD Concentrations in the Retina}
While spectral unmixing can in principle be used to unmix retinal fluorophores the large error limits its efficacy to having the ability to detect the presence of retinal FAD but the concentration can only be quantified with low-confidence. While the goal of this simulation was simply to benchmark a new SFLIM unmixing method discussed in the next chapter it could be made more representative of a spectral imaging experiment by considering off-the-shelf filters which have non-rectangular spectral transmission functions that are unity in the pass band and 0 elsewhere. Further, with the discussed unmixing methods suffering from high errors in recovering the relative abundance of FAD it highlights the fundamental questions - ``What benefit does fluorescence lifetime bring to assessing retinal health,'' and ``Does its benefit outweigh the additional noise incurred in each measurement bin.'' These two questions are explored 
\FloatBarrier
\section{Conclusions}
\textbf{To map the concentration of FAD in the retina, a robust unmixing method is needed to overcome the high noise, and large overlap in fluorescence emission spectra and fluorescence lifetimes.}
In this chapter, the feasibility of measuring FAD concentrations in retina using three unmixing methods based on: fluorescence lifetimes, spectrally-resolved fluorescence lifetimes; and just spectral imaging. 
Simulations were constructed with linear combinations of the retinal fluorophores AGE, A2E, and FAD with Poissionian noise applied to simulate the effects of photon fluxes experience\textbf{d} in \textit{in-vivo} SFLIO imaging of the retina. The fitting-based approach was highly sensitive to noise due to the similar lifetimes of the 3 retinal fluorophores and the 6-exponential fit required to model their decay properties (\cref{sec:lifeunmix}). Even at unrealistically high photon fluxes of \num{e12} photons this method showed prohibitively high errors in the recovered fluorophore concentrations and was deemed unsuitable. \textbf{In principle, a 3-exponential fit using the amplitude, $\langle\tau\rangle_{A}$, or intensity weighted lifetimes, $\langle\tau\rangle_{I}$, of FAD, AGE, and A2E, could be adopted to simplify the fitting model and reduce the RMS error in recovered abundances}. While this could reduce the misclassification of short and long lifetime components for each fluorophore it would not reduce the number of photons required for reliable unmixing to what is feasible in clinical practice (\numrange{e3}{e5} photons). 
The reported phasor SFLIM technique~\cite{scipioni2021phasor} was also investigated and was also found to have high error in recovered fluorophore concentrations. At low photon fluxes of \num{e5} the algorithm would misidentify the unmixed fluorophores meaning AGE could be \textbf{identified} as FAD which would introduce large errors in the the recovered concentration of FAD. Under the phasor transform the high spectral overlap and decay properties of AGE, A2E, and FAD resulted in the endmembers not being well separated in phasor space or forming prominent vertices for the unmixing procedure to identify (\cref{fig:phasorresultnoiseless}). 
To provide a benchmark for the unmixing method developed in~\cref{chap:tensSFLIM} a set of spectral bands were optimised for a matrix inversion spectral technique using matrix condition number related metrics (\cref{sec:specunmix}). The performance of this set of 6 detection bands was simulated and found to yield good unmixing performance at high relative fluorophores concentrations but at low concentrations of FAD this error rises to \qty{91.8}{\percent}. This motivates the need for a better unmixing technique that is sensitive to small changes in concentration of key retinal biomarkers. In the next chapter a new SFLIM unmixing method is explored, utilising the same matrix-inversion approach of spectral unmixing to enable linear unmixing of SFLIM datasets, with the aim of overcoming this problem of high error at low concentrations of retinal fluorphores.
