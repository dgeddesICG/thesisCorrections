...
\end{equation}
Generally, $k$ digits of precision in the inverted matrix are lost given a condition number $\propto \log_{10}(\kappa(\mathbf{S}))$~\cite{cheney2012numerical}. For recovering the abundances of retinal fluorophores, which will vary between  \numlist{0; 1}, a condition number of $\kappa(\mathbf{S}) < \num{e2}$ is required to have a desired error of less than \qty{10}{\percent} to enable more subtle changes in fluorophore concentrations to be confidently measured. However, simply measuring the presence of a retinal FAD would be sufficient to show SFLIM could be a viable technique for longitudinal monitoring of retina health.
\FloatBarrier
\subsection{Optimisation of Spectral Detection Bands for\\ Recovering Concentrations of Retinal Fluorophores}
The condition number was used as a metric for finding the optimal location and number of detection bands. While narrow, contiguous bands do give higher spectral resolution each band exhibits lower SNR or increased $\nicefrac{\delta \mathbf{M}}{\mathbf{M}}$ and longer image acquisition times. For this proposed spectral-imaging scheme, a system of \num{6} bands, shown in \cref{fig:retspectrabands}, was found using an exhaustive search that minimised the condition number $< 10$ of all 40 contiguous filters with width \qty{5}{\nm} spanning a wavelength range of \qtyrange{500}{700}{\nm} giving $\binom{40}{6} \approxeq \num{3.8e6}$ possible groups of 6:
\begin{equation}\label{eq:nchoosek}
    \binom{n}{m} = \frac{n!}{k!(n-k)!},
\end{equation}
where $n = 40$ is the total number of unique filter locations, and $k = 6$ is the desired number of filters. The search gave an optimal combination of filters with cut-on wavelengths of \qtylist{500;535;630;640;655;660}{\nm} and a condition number of $\kappa = \num{8.101}$ indicating an at most one digit of precision will be lost. Surprisingly the optimisation gave a set of filters that are located primarily where AGE has the contribution to the fluorescence signal although this in the wavelength range  where AGE, and FAD appear equally but may continually cross each other. These subtle variations in spectra could be leveraged for improved unmixing capabilities but would rely on low-noise to be confident that small variations in the measured spectra are due to spectral variation in the AGE and FAD. With the benefit of hindsight, using wider spectral bands or using the specifications of commercially available bandpass filters would have given a more accurate assessment of the capabilities of spectral unmixing for quantifying retinal FAD. However, for the purposes of providing a benchmark for the SFLIM unmixing method proposed in \cref{chap:tensSFLIM} this is sufficient, in that an optimised SFLIM unmixing method should be as accurate as a sub-optimal spectral umixing method.
...
\end{equation}
where $N$ is the number of samples, and $\epsilon$ is the difference between the ground truth abundance and the recovered abundance. For noiseless data - shown in \cref{fig:spectunmixoptbands}a, are on the order of \qty{e-12}{\percent} however when shot noise, shown in \cref{fig:spectunmixoptbands}b, is applied the average error in recovering the abundance of FAD rises to \qty{91.8}{\percent} for low concentrations.
\begin{figure}[htb]
    \begin{subfigure}[b]{\textwidth}
        \centering
...
\end{figure}
For large relative concentrations of FAD - and for the other retinal fluorophores - the RMS error is low implying that if FAD is the dominant fluorophore in retina then spectral unmixing could be capable of detecting changes in FAD commensurate with those expected from changes in metabolic health. Since the retinal fluorophores, AGE, FAD, and A2E have not been quantitatively measured then this cannot be relied on.

\FloatBarrier
\subsection{Appraising the Feasibility of Using Spectral Imaging for Measuring FAD Concentrations in the Retina}
While spectral unmixing can in principle be used to unmix retinal fluorophores the large error limits its efficacy to having the ability to detect the presence of retinal FAD but the concentration can only be quantified with low-confidence. While the goal of this simulation was simply to benchmark a new SFLIM unmixing method discussed in the next chapter it could be made more representative of a spectral imaging experiment by considering off-the-shelf filters which have non-ideal transmission that are unity in the pass band and 0 elsewhere. Further, with the discussed unmixing methods suffering from high errors in recovering the relative abundance of FAD it highlights the fundamental questions - ``What benefit does fluorescence lifetime bring to assessing retinal health,'' and ``Does its benefit outweigh the additional noise incurred in each measurement bin.'' These two questions are explored 
\FloatBarrier
\section{Conclusions}
To quantify FAD concentrations in the retina, a robust unmixing method to overcome the high noise, and large overlap in fluorescecne emission spectra and fluorescence lifetimes. In this chapter, the feasibility of measuring FAD concentrations in retina using three unmixing methods based on: fluorescence lifetimes, spectrally-resolved fluorescence lifetimes; and just spectral imaging. 
Simulations were constructed with linear combinations of the retinal fluorophores AGE, A2E, and FAD with Poissionian noise applied to simulate the effects of photon fluxes experience in \textit{in-vivo} SFLIO imaging of the retina. The fitting-based approach was highly sensitive to noise due to the similar lifetimes of the 3 retinal fluorophores and the 6-exponential fit required to model their decay properties (\cref{sec:lifeunmix}). Even at unrealistically high photon fluxes of \num{e12} photons this method showed prohibitively high errors in the recovered fluorophore concentrations and was deemed unsuitable. In principle, a 3-exponential fit using the amplitude weighted average lifetime, $\langle\tau\rangle_{A}$, or intensity weighted average lifetimes, $\langle\tau\rangle_{I}$, given in \cref{tab:retlifetimes}, as reference lifetimes. While this could reduce the misclassification of short and long lifetime components for each fluorophore it would not reduce the number of photons required for reliable unmixing to what is feasible in clinical practice (\numrange{e3}{e5} photons). 
