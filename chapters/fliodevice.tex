...
To register images between the sCMOS and FLIMera detectors first the mapping between the two detectors must be found such that a pixel on the FLIMera can be mapped to its corresponding pixel on the sCMOS camera. The affine transform describes this mapping in terms of magnification, $\mathbf{M}$, rotation, $R$, and translation $T$ and can be used since the transformation between the detectors and motion on the retina is rigid i.e parallel lines are preserved between the two cameras.
