...
\footnotetext{The OPTOS California OPTOS 200tx retinal imaging systems can record images with a \SI{200}{\degree} FOV equivalent to \SI{82}{\percent} coverage of the retinal surface~\cite{kato2019quantitative}}
...
In performing spectrally resolved fluorescence lifetime measurements the recorded data will be influenced by the non-uniform spectral response of the imaging optics within the SFLIO device arising from the various anti-reflective coatings amongst other sources as well as the non-rectangular transmission functions of the spectra filters. To ensure accurate recover of fluorophore concentrations in the eye the spectral sensitivity of the system was measured to act as a calibration in the fluorophore discrimination process.
For the SFLIO device the effective spectral response for each spectral band can be obtained from a two sets of measurements: a broadband Xenon white-light source is directed into the objective lens of the SFLIO device and for each spectral filter the spectra the transmitted spectra is measured with a spectrometer where the detector head is positioned in the place of the fundus camera; and a direct measurement of the light source to remove is influence from the previous measurement. 
Mathematically the transmission function, $F_{n}$ for each of the $n$ band are recovered using \cref{eq:filttrans} where $I_n{\lambda}$ is the spectral measurement of the source through the fundus camera and $\alpha(\lambda)$ is the direct measurement of the light sources. 
\begin{equation}\label{eq:filttrans}
    F_{n}(\lambda) = \frac{I_{n}(\lambda)}{\alpha(\lambda)}
\end{equation}
This direct measurement of the light source serves the purpose of cancelling out the spectral sensitivity of the spectrometer, $D(\lambda)$, as well as the spectral response of the light source $S(\lambda)$.
\begin{equation}
