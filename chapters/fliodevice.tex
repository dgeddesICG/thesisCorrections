...
The S-FLIO device is based upon an off-the-shelf fundus camera (Topcon TRC 50-DX) which is ordinarily capable of recording images of the retina with an FOV of up to \SI{50}{\degree}
\subsubsection{Un-Modified TOPCON TRC-50dx}
\begin{itemize}
    \item Un Modified Camera
    \begin{itemize}
        \item Start off with an imaging system with good optical performance
        \item Greatly simplifies the design process - much much fewer optics to optimise
        \item Modifying existing equipment also simplifies the process of turning it into a commercial product - sold as an add on / upgrade of existing equipment
    \end{itemize}
    \item The TOPCON TRC 50-DX
    \begin{itemize}
        \item Capable of recording 50degree FOV images of the retina with 3 FOV setting 20,35,50 degrees
        \item Multiple image modalities Bright-field, auto-fluorescence, and dye fluorescence - fluorescein angiography, iodocyancine green imaging
        \item compensation for different dioptres
        \item Xenon flash lamp for white light imaging, W filament lamp for inspection mode aided by green leds that come in to focus on the centre of the cornea when instrument is positions correctly with respect to the eye
        \item annular illumination
        \begin{itemize}
            \item Gullstrand principle - annular illumination minimised the overlap between the imaging and the illumination volumes - increases image quality while also slightly reducing the influence of lens fluorescence
        \end{itemize}
    \end{itemize}
\end{itemize}
\subsubsection{S-FLIO Imaging Requirements}
\begin{itemize}
    \item Purpose is to enhance chemical sensing capabilities in retinal imaging using spectrally resolved fluorescence lifetime imaging
    \item Photon flux is too low to register images on a frame by frame basis with the FLIMera alone - use a standard science grade detector to record high - quality images to extract the necessary registration.
    \item Due to the low fill factor, and small detector size (192, 126) the inspection mode will illuminate a smaller area of around 35degrees. For ultrawidefield or widerfield imaging the patient can then scan their eye around
    \item The retina will be illuminated using a 78 MHz laser to match the input requirements of the FLIMera. By using a super-continuum source we can tune the excitation wavelength using an AOTF enabling increased selection of key retinal fluorophores, and mitigation of lens fluorescence.
    \item The ultimate commercial consideration for this technique is imaging patients with short image acquisition times at eye-safe exposure levels - discussed in the section about eye safety. OPTOS systems acquire images in less than a second whereas reported FLIO measurements require 90s per spectral channel. Long FLIO integration times are driven be the low photon flux from the TCSPC method and the feint fluorescence signal produced by the retina. 
\end{itemize}
\subsubsection{Modification - Illumination Arm}
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
\subsubsection{Modification - Imaging Arm}
\begin{itemize}
    \item for the imaging arm a 90:10 beamsplitter is used in conjunction with 3 SLR lenses to relay the image produced by the fundus camera to a Thorlabs Kiralux 8.9MP sensor which will be used to record images in either brightfield or in fluorescence for coregistration purposes and the Horiba FLIMera to record FLIM images. A linear cassette style filter wheel is fitted either in front of the 
...
The translational shifts in the fluorescence intensity images are then converted to equivalent shifts on the FLIMera using Eq.~\ref{eq:FLIMerashifts} and applied to FLIMera histograms that cover the same time period as the sCMOS acquisition, $T = 1/\SI{10}{\hertz} = \SI{100}{\ms}$. When applying the shifts to each histogram a pixel mask is applied to eliminate the effect of "hot" pixels and faulty pixels on the fluorescence decay. This yields a single $XYT$ - data cube spanning the entire \SI{2}{\minute} acquisition period that as shown in integrated intensity images in Fig~\ref{fig:FLIMerareg}d is now free of motion artefacts and the vascular bundles can be clearly resolved albeit with a lower resolution when compared to the fluorescence intensity images shown in Fig.~\ref{fig:sCMOSreg}. Furthermore, in these intensity images, motion of the sample causes features that would ordinarily be imaged by a masked faulty pixel (See Fig.~\ref{fig:FLIMerareg}b) in a still sample now illuminates a different. Typically this would result in an uneven image since every pixel in the final image would be not be masked an equal number of times. This is accounted for by multiplying each pixel grey value by the factor, $\Gamma$, which weights the total number of frames, $N$, in a sequence with the number of times that pixel is masked, $M$ to yield the high quality image shown in Fig.~\ref{fig:FLIMerareg}d.
