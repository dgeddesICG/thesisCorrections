...
To register images between the sCMOS and FLIMera detectors first the mapping, between the two detectors must be found such that a pixel on the FLIMera can be mapped to its corresponding pixel on the sCMOS camera. This mapping can be modeled as an affine transform (\cref{eq:mapping,eq:affine}) - since movement of the retina can be considered as rigid - which is compsed of four other principle transformation: Magnification, $\mathbf{M}(M_{x},m_{y})$; Rotation, $\mathbf{R}(\theta)$; Translation, $\mathbf{T}(\Delta x, \Delta y)$; and Shear, $\mathbf{S}(h_{x}, h_{y})$ (See Eq.~\ref{eq:TransMatrices}). For the FLIO system Shear was not significantly present in images and is omitted from the image registration process.
...
\subsubsection{Phase Cross-Correlation Image Registration}
While feature based registration algorithms are currently employed for the purpose of registering retinal images these techniques are predominately used on collections of images with similar resolution, field of view, and magnification~\cite{chanwimaluang2006hybrid,faisan2011scanning}.  For registering low-resolution images - polluted by salt-and-pepper noise as well as numerous dead and hot pixels - to high-resolution brightfield images the phase cross-correlation based Fourier-Merlin algorithm is utilised \cite{padfield2011masked}.
\\
For a pair of images that are identical except for a offest i.e $I_{1}(x,y) = I_{2}(x+\Delta x, y + \Delta y)$ this translation can be found by computed from the location of the peak in the cross-correlation between two images.
Similarly to find the magnification and rotation between a pair of images the previous approach albeit the images are FFT'd and then a log-polar transform is applied (\cref{eq:logpolar}). Under these transforms the effects of magnification and rotation now manifest as a translation in the $\rho$ and $\xi$ axes of the image.
\begin{gather}
