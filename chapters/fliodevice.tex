...
A detailed description of the procedure used determine the multiple safe exposure limits are listed in App. \textcolor{blue}{Need to figure how to pap the safety assessment into the appendix} however for both standards the exposure limits, the measured exposure, and the resulting safety factor are given and defined here.
\paragraph*{\citeauthor{ISO15004}\\}
In the ISO standard the SFLIO instrument is evaluated as a Group 1 device where the exposure limits for this classification are defined in terms of a wavelength dependent irradiance pertaining to each section of the eye - retina, cornea and lens, and the anterior segment. For each of these sections, where relevant, there are also weighting functions applied to account for the photothermal and photochemical damage mechanisms shown in
\begin{figure}
    \centering
