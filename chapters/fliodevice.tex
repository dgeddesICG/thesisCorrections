...
Definitively measuring the MTF of an imaging system can be difficult and time consuming task \textcolor{blue}{add better intro sentences for why we're only doing an estimate}. For the purpose of this project only estimates of the MTF are required and so the knife-edge test - often referred to as the slanted-edge test in literature - was chosen for its simplistic experimental and analysis procedure \cite{burns2000slanted, ISO12233}. Briefly, this method allows the measuring of spatial resolution of a digital imager using an image recorded of a hard-edge positioned in the image plane and rotated a few degrees ($<\SI{10}{\degree}$) either off-vertical or off-horizontal. Line profiles through the edge are then aligned to produce a super-sampled Edge-Spread Function (ESF) where the spatial resolution is estimated through either the LSF (Eq.~\ref{eq:ESF}) or the MTF calculated from the LSF.
\\
...
In the analysis of the knife edge images, the location of the edge within the image is first estimated by extracting line-profiles through the edge, and finding the peak of the absolute value of the derivative. Using a straight line fitted through these edge locations the line-profiles are aligned and binned with subpixel accuracy to yield the ESF. As above, the LSF and MTF can now be computed from the ESF. The spatial resolution of the system is determined by the FWHM of a Gaussian fitted to the LSF and is calibrated using images recorded of a Allen key positioned at the retinal plane of the phantom eye (see Appendix. \textcolor{blue}{write brief appendix showing results of the allen key experiment})
...
\begin{figure}
    ...
    \end{annotatedFigure}
    \caption{From the recorded knife edge image using the sCMOS detector (a) the edge is detected and fitted (b) using the gradient of the edge profiles, shown in (c). The edge profiles are aggregated to form the ESF (d). The LSF is determined from the ESF using Eq.~\ref{eq:ESF} and fitted with a Gaussian. The MTF, shown in (f), is then calculated from this LSF using Eq.~\ref{eq:LSFtoMTF} and is plotted with the diffraction limit of the eye.}
    \label{fig:FIMTFresult}
\end{figure}

\begin{figure}
    \centering
    \begin{annotatedFigure}{\includegraphics[width = \textwidth]{figures/sflio-device/FLKnifeEdge.pdf}}
    \end{annotatedFigure}
    \caption{Caption}
