...
\begin{figure}
    ...
    \end{subfigure}
    \caption{For FLIO imaging a Topcon TRC 50DX fundus camera is modified to allow excitation of fluorescence using a pulsed, supercontinuum white light source. a) shows the modifications made to the existing illumination optics to allow for fluorescence imaging over a $\approx\qty{20}{\degree}$ field of view. b) shows the effective illumination from the laser source (blue) and the existing tungsten inspection lamp used for aligning the patient/volunteer during imaging.}
...
% Using a power meter the average power meter, $P_{AVG}$ was measured over the entire excitation band as \qty{1.1}{\milli\watt}. The energy per pulse, $E_{pulse}$ is then:
% \begin{align*}
%     E_{pulse} &= \frac{P_{AVG}\cdot\qty{1}{\second}}{f} = \frac{\qty{1.1e-3}{\watt}\cdot\qty{1}{\second}}{\qty{80e6}{\hertz}}\\
%     E_{pulse} &=\qty{1.375e-11}{\joule}
% \end{align*}

From Table 3 the AEL for a single pulse for a pulse length of \qty{100}{\pico\second}, and wavelength between \qty{450}{\nm} and \qty{500}{\nm} is:
\begin{align}
    AEL_{single} = \qty{7.7e-8}{\joule}\\
\end{align}
Since $\nicefrac{1}{f} < T_{i}$ then all the pulses within this time window of $T_{i} = \qty{5e-6}{\second}$ must be grouped together:
\begin{align}
    AEL_{T_{i}} = \num{7e-4}\cdot T_{i}^{0.75}\unit{\joule} = \qty{7.4e-8}{\joule}
\end{align}
The AEL for pulses over a duration $T = \qty{100}{\second}$ averaged together to calculate the $AEL_{T}$ term:
\begin{align}
    AEL_{T} &= \num{3.9e-5}\cdot C_{3}\unit{\watt}\\
    AEL_{T} &= \num{3.9e-5}\cdot 10^{0.02(468-450)}\unit{\watt}\\
    AEL_{T} &= \qty{8.9e-5}{\watt}
\end{align}
As a single pulse this in terms of the energy deposited in a single 'pulse'
\begin{align}
    AEL_{s.p.T} &= \frac{AEL_{T}}{f} =\frac{\qty{8.9e-5}{\watt}}{\qty{80e6}{\hertz}}\\
    AEL_{s.p.T} &= \qty{1.1e-12}{\joule}
\end{align}
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|}
    \hline
    \textbf{Symbol} & \textbf{Description} & \textbf{Value} (\unit{\joule})\\
    \hline
    ...
    \caption{Accessible Emissions Limits (AEL) pertaining to a Class 1 laser system with a pulse duration of $<\qty{100}{\pico\second}$ and wavelength of \qty{480}{\nm} }
    \label{apptab:aelsum}
\end{table}

\subsection{Measurement of Accessible Emissions of the system}
The accessible emission from the system - or the energy deposited onto the retina - must be measured under two conditions for retinal imaging devices: 1) Under normal use for recording retinal images and 2) when the patient / volunteer moves \qty{100}{\mm} from the ideal/usual imaging position. (See Condition 3 in \cref{apptab:conditions}). Condition 3 captures the risk to retinal health from a diverging source of light focusing onto a potentially diffraction limited sport size The classification of the system is then determined using the most restrictive condition - the lowest AEL coupled with the highest measured power.



\subsubsection{Spectral Measurements}
The NKT supercontinuum laser source used in our system has the potential to cause severe phototoxic, and photothermal damage due to its broadband emission - as shown in \cref{subfig:superk}. For the excitation of flourophores the this broadband emission is reduced down to a single \qty{11.5}{\nano\metre} FWHM band using an AOTF. The spectral output of the system is measured using a Ocean Optics Spectrometer to ensure non-desirable wavelengths are properly attenuated as shown in \cref{subfig:fcspectra}. The FWHM of the source is \qty{11.5}{\nano\metre} which is sufficiently narrow that it can be treated as a single wavelength for the purposes of measuring optical power and determining the accessible emission limits. 
...
The accessible emission of the system is evaluated using an optical power meter to measure the optical power emitted from our system under two conditions - 1) Normal measurement conditions and 2) where the volunteer/ patient has moved away from the usual imaging position by \qty{100}{\milli\metre} - this captures the risk of diverging light focusing down into a diffraction limited point. 
\\
...
The optical power was measured as $P_{AVG} = \qty{46}{\micro\watt}$ and the angles $\alpha$, = \qty{1077}{\milli\radian} and $\gamma = \qty{259}{\milli\radian}$ were calculated. 
Under these conditions the energy per pulse form our system can be calculated as simply:
\begin{align}
    E_{pulse} &= \frac{P_{AVG}}{f} = \frac{\qty{46e-6}{\joule}}{\qty{80e6}{\hertz}}\\
    E_{pulse} &= \qty{5.75e-13}{\joule}
\end{align}
Under Condition 3 the optical power was measured by positioning the \qty{7}{\milli\metre} aperture of a mechanical eye \qty{100}{\milli\metre} away from the normal imaging position and position the detector head of a optical power meter at the retina. Due to the diverging nature of the beam out of the objective lens of the fundus camera, the measured optical power is considerably lower - $P_{AVG,3} = \qty{1.3}{\micro\watt}$. Since the angular subtense, $\alpha_{3}$ is still above the stated value of $\alpha_{max} = \qty{100}{\milli\radian}$, the calculated AEL's do not change

\begin{align}
    E_{pulse,3} &= \frac{P_{AVG,3}}{f} = \frac{\qty{1.3e-6}{\joule}}{\qty{80e6}{\hertz}}\\
    E_{pulse,3} &= \qty{1.625e-14}{\joule}
\end{align}
...
\begin{table}[htbp]
    ...
    \multirow{2}{*}{}
         & \bm{$\alpha$} & \bm{$\gamma$}\\
         &$(\unit{\milli\radian})$& $(\unit{\milli\radian})$\\
         \hline
         \textbf{Condition 1}& 1077 & 259\\
         \textbf{Condition 3}& 372 & 119\\
         \hline
    \end{tabular}
    \caption{Angular subtense $\alpha$, and angle of acceptance, $\gamma$, of FLIO system under Condition 1 - normal imaging conditions, and Condition 3 - where the patient has moved \qty{100}{\milli\metre} away}
    \label{apptab:angles}
\end{table}
...
\begin{table}[htbp]
    ...
    \resizebox{0.98\textwidth}{!}{
        ...
        &\textbf{stop}&&&\textbf{limiting aperture}&\\      \unit{\nano\metre}&\unit{\milli\metre}&\unit{\milli\metre}&&\unit{\milli\metre}&\unit{\milli\metre}\\
        \hline
        $< 302.5$ & - & - && $1$ & $0$\\
        \hline
        ...
        &&& 5.4.1& & \\
        \hline 
        \multirow{3}{*}{$\geq 1400\,\text{to}\, 4000$} & $7 \times \text{Condition 3}$ & $2000$& See Note 1 under & $1 \,\text{for}\, t \leq \qty{0.35}{\second}$ & $100$\\
        &&& 5.4.1 & $1.5 t^{\nicefrac{3}{8}}\,\text{for}\, \qty{0.35}{\second}< t < \qty{10}{\second}$ & \\
        &  &  &  & $3.5 \,\text{for}\, t \geq \qty{10}{\second}\, (t \,\text{in}\, \unit{\second})$& \\
        \hline
        \multirow{3}{*}{$\geq 4000\,\text{to}\, \num{1e5}$} & - & - &  & $1 \,\text{for}\, t \leq \qty{0.35}{\second}$ & $0$\\
        &  &  &  & $1.5 t^{\nicefrac{3}{8}}\,\text{for}\, \qty{0.35}{\second}< t < \qty{10}{\second}$ & \\
        &  &  &  & $3.5 \,\text{for}\,t \geq \qty{10}{\second}\, (t \,\text{in}\, \unit{\second})$& \\
        \hline
        $\geq \num{1e5} \,\text{to} \, \num{1e6}$ & - & - & & $11$ & 0\\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|p{6cm}|c|c|c|c|c|}
        ...
        \hline
        Weighted retinal irradiance & $E_{A-R}$ & $\sum_{350}^{700}E(\lambda)A(\lambda)\Delta\lambda$ & $220$ & $\unit{\micro\watt\per\centi\metre^{2}}$ & $5.3.1.3a$ \\
        \hline
        Unweighted corneal and lenticular infrared irradiance & $E_{IR-CL}$ & $\sum_{770}^{2500}E(\lambda)\Delta\lambda$ & $20$ & $\unit{\milli\watt\per\centi\metre^{2}}$ & $5.3.1.4$ \\
        \hline
        Unweighted anterior segment visible and infrared radiation irradiance & $E_{VIR-AS}$ & $\sum_{380}^{1200}E(\lambda)\Delta\lambda$ & $4$ & $\unit{\watt\per\centi\metre^{2}}$ & $5.4.1.5$\\
        \hline
        Weighted retinal visible and infrared thermal irradiance & $E_{VIR-R}$ & $\sum_{380}^{1400}E(\lambda)R(\lambda)\Delta\lambda$ & $0.7$ & $\unit{\watt\per\centi\metre^{2}}$ & $5.3.1.3a$ \\
        \hline
    \end{tabular}
    }
...
With the laser source operating at maximum power, there is an optical power of $P_{M} = \qty{1.1}{\milli\watt}$, emitted from our system which is incident on the retina. The Weighted retinal irradiance, $E_{A-R}$, for this scenario is then $E_{A-R} = \qty{4245}{\micro\watt\per\centi\metre\squared}$ - exceeding the safe exposure limit by $\approx 20 \times$. A neutral density filter, of $\text{OD} = 1.3$ is fitted to attenuate the light emitted from the system to safe levels.
The attenuated power is measured using the optical power meter to be $P_{M} = \qty{46}{\micro\watt}$. The various irradiances are calculated and shown in \cref{apptab:cwvalues}:
...
\begin{align}
    \alpha &= 2\arctan{\bigg(\frac{d_{source}}{2z_{source}}\bigg)} = 2\arctan{\bigg(\frac{\qty{55}{\milli\metre}}{2\cdot \qty{46}{\milli\metre}}\bigg)} = \qty{1077}{\milli\radian}\\
    \gamma &= 2\arctan{\bigg(\frac{d_{source}}{2z_{source}}\bigg)} = 2\arctan{\bigg(\frac{\qty{12}{\milli\metre}}{2\cdot \qty{46}{\milli\metre}}\bigg)} = \qty{259}{\milli\radian}
\end{align}
For Condition 3 (see Condition 3 in Table \ref{apptab:conditions}) the volunteer moves \qty{100}{\milli\metre} away from the typical imaging position i.e $d_{source} = \qty{46}{\milli\metre} + \qty{100}{\milli\metre} = \qty{146}{\milli\metre}$
The angular subtense, and angle of acceptance, $\alpha_{3}$ and $\gamma_{3}$ are then:
\begin{align}
    \alpha_{3} &= 2\arctan{\bigg(\frac{d_{source}}{2z_{source}}\bigg)} = 2\arctan{\bigg(\frac{\qty{55}{\milli\metre}}{2\cdot \qty{146}{\milli\metre}}\bigg)} = \qty{372}{\milli\radian}\\
    \gamma_{3} &= 2\arctan{\bigg(\frac{d_{source}}{2z_{source}}\bigg)} = 2\arctan{\bigg(\frac{\qty{12}{\milli\metre}}{2\cdot \qty{146}{\milli\metre}}\bigg)} = \qty{82}{\milli\radian}
\end{align}
...
The power meter applies a wavelength dependent bias function to overcome the non-uniform sensitivity of the detector but since the spectral output of our system is sufficiently narrow ($\text{FWHM} < \qty{11.5}{\nano\metre}$) we can set this bias to be centred on our excitation ($\lambda = \qty{468}{\nano\metre}$)
...
\end{align}
With $z_{eye} = \qty{17}{\milli\metre}$, $z_{source} = \qty{46}{\milli\metre}$, and $d_{source} = \qty{12}{\milli\metre}$ the area of the retina illuminated is:
\begin{align}
    A_{eye} &=\qty{0.15}{\centi\metre\squared}
\end{align}

The area illuminated on the cornea is used in the calculation of $E_{IR-CL}$ and $E_{VIR-AS}$ and is determined by measuring the diameter of the spot illuminated in the corneal plane under normal imaging conditions. With an illuminated spot size $d_{cornea} = \qty{1.5}{\milli\metre}$:
\begin{align}
    A_{cornea} = \frac{1}{4}\pi d_{cornea} = \qty{0.0177}{\centi\metre\squared}
\end{align}
