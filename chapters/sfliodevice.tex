...
\end{figure}
The modifications made to the illumination arm of the fundus camera for tuneable excitation of retinal fluorescence are described in section{}. Likewise, changes made to the imaging arm to enable spectral-fluorescence lifetime measurements are discussed in section{}. These modifications are highlighted in (see \cref{fig:sflioopticsfull}).
...
The SFLIO device excites retinal fluorescence using a pulsed-supercontinuum white-light source (NKT SuperK Ex-12W and Fianium blue-tuned source) tuned into 8 narrow bands ($\text{FWHM} = \SI{1}{\nm}$) with an Acousto-Optically Tuneable Filter (AOTF). Tuning the excitation wavelength, while not thoroughly explored in this thesis, would allow for retinal fluorophores to be targeted using their unique excitation spectra. The fibre output of the AOTF is coupled into the illumination path of the fundus camera by positioning a dichroic mirror in the place of a filter wheel. Wavelengths shorter than \SI{505}{\nm} are reflected into the illumination path and unwanted longer wavelengths are transmitted into the chassis of the fundus camera and absorbed. To match the Numerical Aperture (NA) of the fibre output of the laser the beam is collimated ($\phi = 1", f = \SI{100}{nm}$) and refocused ($\phi = 1", f = \SI{150}{nm}$) to a point at the perimeter of the annular stop $A_{1}$ of \cref{fig:sfliooptics}. This annular stop is conjugate to the annulus projected on the cornea and results in the excitation of fluorescence over $\SI{20}{\degree}$ of the retina (see \cref{fig:illuminationarea}). 
...
The imaging arm of the SFLIO instrument uses the Horiba FLIMera SPAD array (referred to as the FLIMera herein) to record fluorescence lifetime data over a $\cong\SI{12}{\degree}$ field-of-view. A separate scientific camera (Thorlabs Kiralux) to records fluorescence intensity or brightfield images over a \SI{35}{\degree} field-of-view to correct for motion of the retina during the long acquisition period. SLR lenses were set up in a relay configuration to collimate the image ($f=\SI{100}{\mm}$) and re-image onto the FLIMera ($f = \SI{50}{\mm}$ ) and sCMOS camera ($f = \SI{85}{\mm}$). A 2” plate beam splitter transmits \SI{10}{\percent} of the signal to the sCMOS camera and reflects \SI{90}{\percent} to the FLIMera - this maximises the efficiency of the SFLIM measurements without degrading the quality of the registration. Due to the difference in pixel-counts between the two detectors - $\qtyproduct{192 x 126}{\pixel}$ for the FLIMera and \qtyproduct{4096 x 2160}{\pixel} the field-of-view covered by the FLIMera was reduced by a linear factor of $\approx 3$ to improve the spatial resolution of the acquired images with the intention that if SFLIM images of larger areas of the retina the patient can fixate on their eyes away from centre.
...
\end{figure}
The safety thresholds (see \cref{tab:conthresh}) were calculated with the optical power measured when the laser source was set to its maximum output power - $P_{M} = \SI{1.1}{\milli\watt}$ - exceeding the safe exposure limits for a Group 1 device by a factor \num{20}. A neutral density filter with $\text{OD} = \num{1.3}$, calculated using \cref{eq:OD}  was fitted into the illumination arm to attenuate the optical power incident on the retina to eye-safe levels of $P_{M}=\SI{46}{\micro\watt}$ and the safe exposure thresholds were recalculated with a safety factor, $\eta = \nicefrac{\text{Threshold}}{\text{Value}}$.
...
Additionally, this fill factor can be improved upon using various established techniques at the fabrication stage. A micro-lens array would focus light incident within a pixel onto the smaller photosensitive area (see \cref{fig:SPADlayout}b) - increasing fill-factor \SI{42}{\percent} in the case of the FLIMera \cite{henderson2019192}. The fill-factor could also be increased using a multilayered, back-illuminated, architecture (see \cref{fig:SPADlayout}c)  where the photon counting electronics are on a layer beneath the photosensitive area of the pixel.
...
The FLIMera has two acquisition modes: a default mode where a single XYT data cube is constructed over the acquisition window; and the ``raw'' mode where now each photon detection event is streamed from the FLIMera at a rate of \SI{24}{\kilo\hertz} to form ``frames''. These photon records take the form of a 4 byte word which from the least to most significant bit contains the X (\SI{8}{\bit}), Y (\SI{8}{\bit}), and time bins (\SI{10}{\byte}) coordinate of the photon detection with the final \SI{4}{\bit} are used as a frame marker (1111) to denote the next frame in the acquisition or a photon record. This higher frame-rate mode comes at the penalty of larger file where a \SI{2}{\minute} integration time in ``raw'' results in a \SI{25}{\giga\byte} compared to the $\approx\SI{25}{\mega\byte}$. In this project this ``raw'' mode is used to compensate for the motion of the retina throughout the acquisition period (see \cref{sec:motionreg}). 
\FloatBarrier
...
\begin{figure}
    ...
    \end{annotatedFigure}
    \caption{From the recorded knife edge image using the sCMOS detector (a) the edge is detected and fitted (b) using the gradient of the edge profiles, shown in (c). The edge profiles are aggregated to form a super sampled ESF (d). The LSF is then determined from the ESF using Eq.~\ref{eq:ESF} and the spatial resolution of \SI{214}{\um} is determined from the FWHM of a fitted Gaussian (black line). The MTF, shown in (f), is then calculated from this LSF using Eq.~\ref{eq:LSFtoMTF} and is plotted with the diffraction limit of the eye.}
...
\begin{figure}
    ...
    \end{annotatedFigure}
    \caption{From the recorded knife edge image using the FLIMera (a) the edge is detected and fitted (b) using the gradient of the edge profiles, shown in (c). The edge profiles are aggregated to form a super sampled ESF (d). The LSF is then determined from the ESF using Eq.~\ref{eq:ESF} and the spatial resolution of \SI{50}{\um} is determined from the FWHM of a fitted Gaussian (black line). The MTF, shown in (f), is then calculated from this LSF using \cref{eq:LSFtoMTF} and is plotted with the diffraction limit of the eye.}
...
While the objective of this project was not to demonstrate diffraction limited imaging of the retina using the FLIMera - the spatial resolution of the S-FLIO device should be sufficient to resolve common markers of retinal disease such as drusen. Drusen is common marker of AMD (\cref{intro:AMD}) and typically has a diameter on the order of \SI{50}{\um}~\cite{abdelsalam1999drusen}
...
In~\citeauthor{mordant2011validation} the spatial resolution of the system was estimated to be $\approx\SI{30}{\um}$ or $\approx\SI{3}{\pixel}$ by examining line profiles through an image of a \SI{150}{\um} outer-diameter capillary tube and computing the rise time - the number of pixels required to transition from \SI{10}{\percent} to \SI{90}{\percent} of the peak pixel grey-value within the capillary.
...
The sources of this discrepancy in spatial resolution was thought to originate from the 90:10 beam-splitter in the imaging arm and / or the \SI{500}{\nm} long pass emission filter. A chequerboard target (pitch of \SI{10}{\mm}) was positioned at infinity ($>\SI{3}{\metre}$ from the objective lens of the fundus camera) and these sources were systematically removed from the imaging path. The rise-time or the black-to-white transition time was then be computed for each scenario, as shown \cref{fig:sCMOStroubleshooting}, in terms of pixels and yielded a mean rise-time of \SI{4.5}{\pixel}. Since this difference rise-time ($\SI{3}{\pixel}:\SI{4.5}{\pixel}$) is proportionally similar to the discrepancy in spatial resolution ($\SI{30}{\um}:\SI{50}{\um}$) it was then concluded that the optics of the fundus camera or the SLR lenses used in the imaging arm are the limiting factor and not the beam-splitter or emission filter.
