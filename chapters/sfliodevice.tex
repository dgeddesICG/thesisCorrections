...
The FLIMera has two acquisition modes: a default mode where a single XYT data cube is recorded over the acquisition window; and the ``raw'' mode where now each photon detection event is streamed from the FLIMera at a rate of \qty{24}{\kilo\hertz} to form ``frames''. These photon records take the form of a 4 byte word which from the least to most significant bit contains the X (\qty{8}{\bit}), Y (\qty{8}{\bit}), and time bin (\qty{10}{\byte}) coordinate of the photon detection. The final \qty{4}{\bit} are used as a frame marker (1111) to denote the next frame in the acquisition or a photon record. This higher frame-rate mode comes at the penalty of larger file where a \qty{2}{\minute} integration time in ``raw'' mode results in a \qty{25}{\giga\byte} compared to the $\approx\qty{25}{\mega\byte}$ of the default acquisition mode. In this project the high frame-rate ``raw'' mode is used to allow motion of the retina to be compensated using a high resolution brightfield images recorded at \SI{10}{\Hertz}(see \cref{sec:motionreg}).
...
While the objective of this project was not to demonstrate diffraction limited imaging of the retina using the FLIMera - the spatial resolution of the S-FLIO device was measured to be sufficient to resolve common markers of retinal disease such as drusen. Drusen is common marker of AMD (\cref{intro:AMD}) and typically has a diameter on the order of \qty{50}{\um}~\cite{abdelsalam1999drusen} – the minimum feature size resolvable by the S-FLIO system should be smaller than this. The spatial resolution of both detectors also affects the accuracy of co-registration algorithms when compensating for motion of the retina. For a proportional difference in the spatial resolution $\nicefrac{\Delta r_{FLIMera}}{\Delta r_{sCMOS}} \leq M$ will require registration of images within a sub-pixel accuracy (with respect to the sCMOS detector) whereas for $\nicefrac{\Delta r_{FLIMera}}{\Delta r_{sCMOS}} > M$ the registration accuracy requirement is greater than a single pixel.
