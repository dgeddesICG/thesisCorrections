...
In this chapter, a technique for unmixing fluorophores from SFLIM measurements is developed using a least-squares solver for multi-linear systems of equations published in literature~\cite{brazell2013solving}. Next, the detection bands for this unmixing technique are optimised for measuring FAD concentrations in the retina from SFLIM measurements using the condition number. The unmixing performance of the tensor-based method is characterised with simulated mixtures of retinal fluorophores showing decreased errors in recovered FAD concentration when compared to spectral unmixing for equivalent  photon flux. Generalised simulations of dyes with overlapping spectra and distinct lifetimes show that this technique that benefits from the inclusion of lifetime measurements. Further, simulations of gate-SFLIM measurements show it to be a more photon efficient detection scheme than TCSPC-SFLIM for unmixing fluorophores at equivalent photon fluxes.

\FloatBarrier
\section{A Multi-Linear Least-Squares Solver for Unmixing Retinal Fluorophore Concentrations From SFLIM Measurements}
In the previous chapter, existing methods for recovering FAD concentrations from SFLIM measurements were investigated and found to be unsuitable even in low-noise scenarios. For both the fitting based and phasor S-FLIM method their limitations lie with the high overlap of retinal fluorophores in the spectral and lifetime domain.
\\
To address this, a new method has been developed which treats the temporal domain of SFLIM data as a pseudo-`spectra' and extends the matrix inversion approach of standard spectral unmixing to include the temporal domain. This combines the decay profile and emission spectra of the fluorophore to minimise the effects of this high overlap in both domains. The existing Moore-Penrose inverses used in the matrix inversion unmixing technique are only well defined for matrices – endmember tensor in~\cref{fig:tensor} cannot be inverted. To extend this technique for unmixing fluorophore concentration from SFLIM data the work of~\citeauthor{brazell2013solving}\cite{brazell2013solving} was adapted. In their work an algorithm for solving multi-linear systems of equations is described using Moore-Penrose inversions of third-order tensors.


\FloatBarrier
\subsection{Mathematical Aside - Definitions and Nomenclature of Tensor Algebra}\label{subsec:tensdef}
\textcolor{red}{Should this be an appendix?}
...
Operations between tensors are generalised using the Einstein summation or Einstein notation conventions by summing over ``free'' indices which appear only once in an expression and element wise multiplications of entries in ``dummy'' indices that are repeated within an expression. To illustrate this the example in~\cref{eq:matmul,eq:matmulein} show multiplication of two matrices, $\mathbf{A} \in \mathbb{R}^{I \times J}$ and $\mathbf{B} \in \mathbb{R}^{J \times K}$ using standard indexed summations and Einstein notation where summation symbols are conventionally omitted for simplicity and brevity.
\begin{align}
...
\end{align}
To simplify notation, the elements of the tensor transpose,~$\mathcal{D}^{T}$, is notated as $\hat{\mathbf{d}}$ and for indices that are twinned they are denoted as $\tilde{\sigma}$ where they has same length but are not to be treated as ``dummy'' indices.
...
\end{equation}
For a least-squares method (\cref{eq:leastsquaretens}) there is then the minimising function ,$\varphi(\mathbf{x}) = \big\lvert\big\lvert \mathcal{A}\bullet_{3}\mathbf{x} - \mathbf{M}\big\rvert\big\rvert_{F}^{2}$, with a Jacobian, $\nabla\varphi(\mathbf{x})$. An estimator of the relative abundances, $\bar{\mathbf{x}}$ can then be found at the critical point where $\nabla\varphi(\bar{\mathbf{x}}) = 0$ - the local minimum.
...
In this chapter a new method for unmixing FAD concentrations from SFLIM measurements of the retina was developed and the optimal spectral bands for recording these SFLIM measurements were found. The SFLIM unmixing technique is adapted from the Jacobian based minimisation procedure described by~\citeauthor{brazell2013solving}\cite{brazell2013solving}.
After optimising the location and number of spectral bands the performance of the SFLIM unmixing algorithm was then shown to imprFiniove capabilities of quantifying retinal FAD, compared to spectral unmixing, by exhibiting a reduced RMS error - \qty{72.4}{\percent} for SFLIM unmixing opposed to \qty{91.8}{\percent} for spectral unmixing - for equivalent photon fluxes. Finally, the flexibility of the SFLIM unmixing algorithm was briefly explored by comparing the condition number of spectral unmixing, to SFLIM unmixing, and gated-SFLIM unmixing computed when unmixing sets of Gaussian spectra simulated to have either high spectral overlap or highly similar fluorescence lifetimes. These simulations demonstrated a limitation of TCSPC-based SFLIM methods where SFLIM exhibits a lower SNR than a spectral imaging system simply due to the same photon flux being distributed over more shot-noise and read-noise affected measurements. Despite this, the SFLIM unmixing algorithm does outperform traditional spectral unmixing in recovering FAD concentrations with a lower RMS error. A possible solution to this noise issue could be to use a time-gated detection scheme where the $>250$ time bins spanning \qty{47}{\ps} are replaced by two or more \qty{2}{\ns} wide gates. This has the effect of increasing SNR at the cost of temporal resolution but as shown in \cref{fig:sflimunmixingoverlapspectra,fig:sflimunmixingoverlaplifetime} this would not necessarily culminate in poorer unmixing performance. In principle, gated detection schemes also address the two of the major challenges of using TCPSC-SPAD arrays for FLIM: photon-pileup where only one photon can be detected per excitation pulse; and the low-pixel count of the FLIMera where megapixel gated SPAD detectors have been reported~\cite{morimoto2020megapixel}.
\FloatBarrier
