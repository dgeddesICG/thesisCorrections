...
In the previous chapter, the phasor-SFLIM method for recovering FAD concentrations from SFLIM measurements was investigated and due to high overlap between endmembers in phasor space the high-error in recovered fluorophore concentration - regardless of noise. This rendered the technique unsuitable for \textit{in-vivo} measurements of retinal FAD in humans to monitor metabolic health in the eye. For both the fitting based and phasor-SFLIM method their challenges lie with the high overlap of retinal fluorophores in the spectral and lifetime domain.
\\
...
\end{align}
where $\mathbb{A}$, and $\mathbf{B}$, are two matrices such that $\mathbf{A} \in \mathbb{R}^{I \times J}$ and $\mathbf{B} \in \mathbb{R}^{J \times K}$ giving the resultant matrix, $\mathbf{A} \in \mathbb{R}^{I \times J}$. Using the simplified Einstein summation notation the summation symbol is omitted for convenience:
\begin{align}
...
\end{equation}
A function, $\varphi(\mathbf{x}) = \big\lvert\big\lvert \mathcal{A}\bullet_{3}\mathbf{x} - \mathbf{M}\big\rvert\big\rvert_{F}^{2}$, with Jacobian, $\nabla\varphi(\mathbf{x})$, can then be defined to satisfy the minimisation in \cref{eq:leastsquaretens}. An estimator of the relative abundances, $\bar{\mathbf{x}}$ can then be found at the critical point where $\nabla\varphi(\bar{\mathbf{x}}) = 0$ - a local minimum.
...
\end{figure}
It was observed that best filter combination for each metric shown in~\cref{fig:tensoptband}a-c shows overlapping, redundant spectral bands which provided minimal gains in unmixing accuracy at the penalty of increased integration time through having to record more spectral bands. These overlapping filters were consolidated to give sets of four wider band filters - keeping the three narrow bands between \qty{500}{\nm} and \qty{575}{\nm}. The location of this fourth band was then re-optimised using the condition number and $\nicefrac{\sqrt{BW}}{\kappa}$ which gives the result in \cref{fig:tensoptband}d-e.
Whilst this does increase the average RMS error in recovered FAD: from $\epsilon_{FAD} = 0.618$ for six bands to $\epsilon_{FAD} = 0.724$, for the same \num{e5} detected photons. As shown in \cref{tab:SFLIMRMSerrors}, it is still lower than the five band system previously optimised for spectral imaging and as shown in \cref{fig:SFLIM-RetFluor-1e5photons}.
\begin{table}[htp]
    \centering
...
The addition of \textit{in-vitro} experiments would strengthen the conclusions of this chapter. Mixtures of retinal fluorophores could be prepared in capillary tubes and mounted into an eye phantom. The concentration of each fluorophore could then be varied across multiple SFLIM measurements, imaged with the SFLIO device. This would serve as a more rigours way to measure what the smallest change in FAD concentration that can be detected using the SFLIM unmixing algorithm and existing spectral unmixing techniques. Due to time constraints, this experiment was regrettably not carried out.
