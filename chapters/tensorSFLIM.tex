...
In this chapter the an SFLIM unmixing technique is developed using a least-squares solver for multi-linear systems of equations published in literature. Next the detection bands for this unmixing technique are optimised for measuring FAD concentrations in the retina from SFLIM measurements using the condition number. The unmixing performance of the tensor-based method is characterised with simulated mixtures of retina fluorophores showing a decreased errors in recovered FAD concentration when compared to spectral unmixing for equivalent  photon flux. Generalised simulations of dyes with overlapping spectra and distinct lifetimes show that this technique is benefited from the inclusion of lifetime measurements. Further, simulations of gate-SFLIM measurements show it to be a more photon efficient detection scheme than TCSPC-SFLIM for unmixing fluorophores at equivalent photon fluxes.
...
Using this mathematical framework the above SFLIM unmixing method, shown in \cref{alg:sflim} is implemented in \textit{python} using a least-squares minimiser and Einstein Summations to represent the Einstein Products, $\ast_{2}$, and the Tucker Mode Products, $\bullet_{3}$. Using the Jacboian (\cref{eq:jacfunc}) and Estimator function (\cref{eq:xbar}) ensure convergence in fewer iterations to the best-fit parameters and higher confidence the solution represents a global-minima and not a local-minima. Further, this helps compensate for the similarity of endmembers in the $\mathcal{A}$, and the influence of noise. For well-conditioned and low-noise problems the estimator $\bar{\mathbf{x}}$ can be evaluated directly to recover the relative abundance of endmembers without the need for the least-squares minimisation step.
