...
\begin{equation}\label{eq:expgauss}
    I(t) = \frac{A}{\sigma\sqrt{2\pi}}\exp\Bigg(\frac{-(t-\mu)^{2}}{2\sigma^{2}}\Bigg)\Erfc\Bigg(-\frac{\alpha(t-\mu)}{\sigma\sqrt{2\pi}}\Bigg)
\end{equation}
\begin{equation}\label{eq:erf}
    \Erf(z) = \frac{2}{\sqrt{\pi}}\int_{0}^{\infty}\exp(-x^{2})dx
\end{equation}
From the plots below recorded of  Aluminium this Exponentially Modifed approximates the data well

\begin{figure}
    \centering
    \includegraphics[width = 0.8\textwidth]{figures/TCSPC/ExpGaussExample.pdf}
    \caption{. Example TCSPC histogram recorded from a \SI{100}{\ps} \SI{700}{\nm}, pulse of light back scattered off a block of Aluminium where red 'x's denote the detected photon counts, and the black line is the line of best yielded when fitting with a Exponentially modified Gaussian (Eq.~\ref{eq:expgauss})}
    \label{fig:expgauss}
\end{figure}
\FloatBarrier
