...
In the first set of dyes the emission spectra were simulated to be highly overlapping with a FWHM of \SI{15}{nm} and central wavelengths only differing by \SI{2}{nm (CWL = \qtylist{550;552;554}{\nm})} but the respective fluorescence lifetimes were well separable - $\tau = \qtylist{1.50;2.00;2.50}{\ns}$. When examining the condition number of this scenario (see \cref{fig:sflimunmixingoverlapspectra}) predictably the high spectral overlap gives an increased condition number when compared to the SFLIM unmixing approach where this effect is largest for a 4 band system. 
...
Similarly for the second scenario the spectra were simulated to be well-separable with central wavelengths of \qtylist{500;600;650}{\nm} with a FWHM of \SI{15}{\nm} but now the lifetimes were separated by just \SI{100}{\ps} which would ordinarily be difficult to distinguish based upon a fitted lifetime. From this simulation, the condition numbers for all three methods are equal across all spectral band numbers indicating that majority of the information available in the discrimination process is in the spectral domain. While this implies for an equal SNR per channel measurement the SFLIM unmixing approach would yield similar performance to standard spectral unmixing - in an experimental implementation the SFLIM approach would exhibit a lower SNR per measurement ($\lambda \time \Delta t$ measurements) and be more error prone.
