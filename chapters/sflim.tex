...
Since this new method now incorporates the unique temporal decay of each fluorophore this brings about a new dimensions that the fluorophores can be discriminated in. The spectral bands that the SFLIM data is recorded over should be re-optimised since some bands that might not provide much spectral information might give a large amount in the temporal domain relative to other bands. Similarly the different FLIM detection: standard TCSPC FLIM, gated FLIM, and truncated TCSPC FLIM schemes can be optimised using the same approach in Sec.~\ref{sec:specunmixopt} and compared with the heuristically chosen detection bands.
...
\begin{table}
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline  
    \end{tabular}
    \caption{Condition numbers, $\kappa$, for unmixing retinal fluorophores using tensor based and matrix based unmixing methods where: $T$ refers to the tensor based inversion method, $T_{gated}$ referes to the gated FLIM detection scheme consisting of 2 contiguous \SI{2}{\ns} gates, $T_{trunc}$ is a modifcation to the tensor inversion method where teh final \num{100}, typically noisier, time bins are excluded, and finally $S$ is the spectral unmixing method using matrix inversions.}
