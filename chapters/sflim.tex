...
\end{equation}
The rate at which oxygen is metabolised, $Q_{\ce{O2}}$, is computed using ~\cref{eq:o2rate} from the average blood flow through the ophthalmic artery, $Q_{ret}$, and the relative change in saturation of blood entering, ${S\ce{O2}}_{in}$ and exiting it ${S\ce{O2}}_{out}$. The rate of blood flow through the ophthalmic artery was taken as $Q_{ret} = \SI{5}{\micro\litre\per\minute}$ by averaging two reported independent measurements recorded using different methods~\cite{dai2013absolute,riva1985blood}. Further the oxygen saturation - the ratio of oxygenation to deoxygenated haemoglobin in the blood - has been reported as \SI{92.20}{\percent} and \SI{55.60}{\percent} for blood entering and exiting the ophthalmic artery~\cite{dunn2016physiology}.
\begin{equation}\label{eq:o2rate}
    Q_{\ce{O2}} = Q_{ret}({S\ce{O2}}_{in}-{S\ce{O2}}_{out})H
\end{equation}
Using \cref{eq:o2rate} the rate of oxygen metabolisation was estimated to be $Q_{\ce{O2}}=\SI{3.155}{\micro\litre\per\minute}$ where $H=\num{0.285}$ is Hüffners constant which is the volumetric ratio of oxygen to haemoglobin in the blood~\cite{dunn2016physiology}. The rate of FAD production can then be determined by considering that for every metabolised \ce{O2} molecule $k_{FAD}$ = 6 molecules of FAD are produced with an efficiency of $\eta_{FAD} = \SI{1.3}{\percent}$\cite{berg2007biochemistry, ames1992energy}.
\begin{equation}
...
\begin{equation}
    C_{FAD} = \frac{N_{FAD}}{V_{ret}}
\end{equation}


\textcolor{blue}{Add in section summarising our requirements for a successful unmixing method}
\begin{itemize}
    \item FAD is a biomarker that is linked with the metabolising of oxygen in surrounding tissue
    \item quantifying and longitudinally monitoring the concentration of FAD can allow for diagnosis of disease / decline in retinal health at the first signs of biochemical dysfunction - before structural damage occurs to the retina where symptoms of lost vision become apparent
    \item The ground truth value for FAD is not known but we can put some bounds on it and estimate the relative contribution FAD makes to any detected fluorescence signal from the retina
    \item Estimate FAD concentration using similar approach to Heather McEwan did for her undergrad project
    \item use this estimate to link the relative contribution of FAD in the fluorescence signal to its absolute concentration. 
    \item propose suitable error bounds for the unmixed concetration of FAD
\end{itemize}
...
\subsection{To Do}
\begin{itemize}
    \item Abundance maps for noiseless data and data with shot noise (1e4 - 1e5 photons per pixel) to show that Phasor S-FLIM isn't suitable for unmixing retinal fluorophores. 
    \item This is because Phasor S-FLIM is best suited for stained samples where dyes can be chosen such that they are spectrally narrow and well seperated and largely different lifetimes - i.e. the endmembers are far away from each other in phasor space. Retinal fluorophores have similar fluorescence lifetimes and broad overlapping spectra so the performance of the unmixing is severely detoriated.
    \item conclusion: this technique is not suitable for unmixing retinal fluorophores
\end{itemize}
\FloatBarrier
...
\end{align}
\footnotetext{The operator ``$\bullet_{3}$'' represents the Tucker product along the third mode but in the case for the tensor $\mathcal{A}$,and vector $\mathbf{x}$ this numerically similar to a scalar product along the third dimension of $\mathcal{A}$. A more detailed description is given in App.~\ref{app:tensor}}
\begin{figure}[h]
\centering
...
\FloatBarrier
\subsection{Unmixing Retinal Fluorophores}
The abundance maps used in previous section (Sec.~\ref{sec:lifeunmix},~\ref{sec:phasor-sflim},~\ref{sec:specunmix}) can be used to assess the efficacy of this S-FLIM unmixing method when unmixing retinal fluorophores and compare it to Phasor S-FLIM and traditional spectral unmixing using matrix inversions. The condition number can also be used to compare these different unmixing methods as well optimise the number and location of the spectral bands and other detection schemes such as time gated FLIM (Sec.~\ref{sec:gatedflim}) and a scheme where the noisiest time bins are discarded and not used in the unmixing process. This is a similar approach taken where fitting fluorescence decays for TCSPC FLIM data where robustness to noise in the fitting process is exchanged for decreased ability to resolve complex decay models. To simulate gated FLIM data fluorescence decays are first generated then integrated over two contiguous time windows (See Fig.~\ref{figintro:tgflim}) to form the two gated images. For these simulations a gate width of \SI{2}{\ns} was chosen and is commensurate with literature \addcite
