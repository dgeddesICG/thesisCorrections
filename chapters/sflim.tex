...
For a given spatial pixel $I(x,y)$ the fluorescence lifetimes are recorded over multiple discrete spectral bands and the fluorescence decays yielding $I(t,\lambda)$, these decays are then phasor transformed using the first harmonic (Eq. \ref{eq:phasor-g} and \ref{eq:phasor-s} evaluated at $n = 1$) and the spectral and temporal instrument response function is corrected for. The separate  spectral phasors, $g(\lambda)$ and $g(\lambda)$, coordinates are then weighted with principle components analysis (PCA) and fitted to a generalised double logistic function (Eq. \ref{dublog}). In the original published work the authors note that the performance of this method relies on the phasors of the pure endmembers being well separated in phasor space and as such it is best suited for labelled imaging where there is a more control over the spectral, and temporal fluorescence characteristics of the dyes used. Nevertheless, this algorithm was implemented for the application of unmixing retinal fluorophores. 
...
\end{align}
\footnotetext{The operator "$\bullet_{3}$" represents the Tucker product along the third mode but in the case for the tensor $\mathcal{A}$,and vector $\mathbf{x}$ this numerically similar to a scalar product along the third dimension of $\mathcal{A}$. A more detailed description is given in \ref{subsec:tensoraside}}
\begin{figure}[h]
\centering
...
\end{align}
Using this mathematical framework the SFLIM unmixing method can be implemented in a least-minimisation package which will be robust and computationally efficient given the analytical Jacobian, defined in Eq.~\ref{eq:jacfunc}, and a close estimate of the optimal solution defined in Eq.~\ref{eq:xbar}. The algorithm is described in Alg.~\ref{alg:sflim} and is implemented using \textit{einsum} library of Python's \textit{numpy} package for the calculations of the Einstein products $\ast_{2}$ and the Tucker Mode products $\bullet_{3}$:
\begin{algorithm}
    \caption{Algorithm for recovering endmember abundances from spectral FLIM data}
    \label{alg:sflim}
    \begin{algorithmic}
...
\subsubsection{Mathematical Aside}\label{subsec:tensoraside}
