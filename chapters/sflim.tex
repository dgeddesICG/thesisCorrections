...
For a given spatial pixel $I(x,y)$ the fluorescence lifetimes are recorded over multiple discrete spectral bands and the fluorescence decays yielding $I(t,\lambda)$, these decays are then phasor transformed using the first harmonic (\cref{eq:phasor-g} and \cref{eq:phasor-s} evaluated at $n = 1$) and the spectral and temporal instrument response function is corrected for. The separate  spectral phasors, $g(\lambda)$ and $g(\lambda)$, coordinates are then weighted with principle components analysis (PCA) and fitted to a generalised double logistic function (\cref{eq:dublog}). In the original published work the authors note that the performance of this method relies on the phasors of the pure endmembers being well separated in phasor space and as such it is best suited for labelled imaging where there is a more control over the spectral, and temporal fluorescence characteristics of the dyes used. Nevertheless, this algorithm was implemented for the application of unmixing retinal fluorophores. 
...
This algorithm was tested using simulated data sets, similar to those shown in \cref{fig:sflimamaps}, which consisted of an additive mixture of retinal fluorophores without shot noise applied - to represent the theoretical maximal performance of the technique - and with shot noise to simulate large photon fluxes. These data sets were then analysed using the Phasor S-FLIM algorithm, and the errors were analysed using the same metric as before (\cref{eq:recerror}). The blind nature of this unmixing method resulted in the ordering of the unmixed spectra being inconsistent from pixel-to-pixel and this subsequently was addressed by reordering the unmixed spectra based on their relative overlap with the endmember spectra (see \cref{eq:sam}). 
\begin{equation}
    \varphi = \frac{\mathbf{a}\cdot \mathbf{b}}{\lvert \mathbf{a} \rvert\,\lvert \mathbf{b} \rvert}\label{eq:sam}
\end{equation}
Shown in \cref{fig:phasorresultnoiseless}, the phasor based approach suffers from large errors even under noiseless conditions - where no shot-noise has been applied - over all concentrations of FAD but this significantly pronounced at low fluorophore concentrations where the average error is \SI{100}{\percent}. This is due to the high overlap of FAD, and AGE in the both the spectral and temporal domain which results in phasors that are closely located within phasor space and degrades the ability of the algorithm to discriminate them. 
...
Further when shot-noise is applied this poor unmixing performance is repeated. In \cref{fig:phasorresultnoisy}, SFLIM measurements were simulated using 32 evenly spaced spectral bands and a total of \num{e6} photons in the entire measurement. For the recovery of FAD the exceedingly large errors are present at low concentrations and high relative concentrations of FAD resulting in an average error of \SI{224}{\percent}.
...
In literature, this method exhibits good un-mixing performance but only in dyed samples where there is flexibility in the fluorescence lifetime and spectral characteristics of the dyes used to ensure good separation in phasor space \cite{scipioni2021phasor}. The overlapping in the spectral and temporal domain of the chosen retinal fluorphores results in close proximity in phasor space and is evidenced by the poor unmixing performance in the above modelling (\cref{fig:phasorresultnoiseless}). The S-FLIM algorithm was deemed unsuitable for quantifying concentrations of FAD or even simply detecting its presence in retinal autofluorescence and thus was not pursued further.

\subsection{To Do}
\begin{itemize}
    \item Add captions to plots
    \item fix referenceing
\end{itemize}
\FloatBarrier
