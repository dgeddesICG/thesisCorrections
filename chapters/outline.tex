\begin{itemize}
    ...
    \item En-face Fluorescence Lifetime Ophthalmoscope
    \begin{itemize}
        \item Fundus Camera Overview
        \item Modifications - Illumination Arm
        \begin{itemize}
            \item Annular illumination and gullstrand principle
            \item Annular mirror, and conjugate stops to remove corneal back reflections.
            \item Description of supercontinuum laser and AOTF system.
            \item Optical layout of fundus camera
            \item Attempt 1 - injecting light via Xe flash lamp
            \item Attempt 2 - injecting light via the filter wheel
        \end{itemize}
        \item Modifications - Imaging Arm
        \begin{itemize}
            \item Ray trace for SLR lens arrangement
            \item Spectral Channels chosen
            \item Fluorescence Intensity Camera and approx. field of view
            \item SPAD Camera
            \begin{itemize}
                \item SPAD array has 192 X 128 pixels and 47ps temporal resolution. Has fill factor of 13 percent. TCSPC modules on a per pixel basis.
                \item Operating Modes of SPAD
                \begin{itemize}
                    \item Standard H5 Mode - data acquisition is defined by the acquisition time, number of bins etc. Data output is a h5 containing the tcspc histogram for each pixel, approx fwhm, coarse lifetime (using horibas black boxy fitting), and integrated intensity
                    \item Raw H5 Mode - Instead of single tcspc histogram per pixel, instead in raw mode the raw photon records are streamed to a H5 pixel. These photon records take the form of a 4byte string of digits indicating the pixel, and time bin the photon was detected in. This is further discretised into the individual "frames" which have a frequency of 11kHz which allows for building histograms over arbirtary time windows.
                \end{itemize}
                \end{itemize}
            \end{itemize}
        \end{itemize}
        ...
        \begin{itemize}
            \item Description of damage mechanisms when illuminating the retina. Photochemical or phototoxic damage occurs predominately in the shorted wavelengths and results in the generation of free radicals. The immune system then either sees these as foreign and removes the cells or the photorecepotors receive too much energy and die directly. In photo thermal damage light in the red wavelengths causes heating, and cavitation, of the humors at the retinal surface. This causes the retinal surface to be irreparably destroyed.
            \item different safety documents were used to evaluate the safe emission levels of imaging at our excitation wavelength. 
            \item detail calculations and measurements
        \end{itemize}
    \end{itemize}
