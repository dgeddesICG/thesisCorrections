...
\end{figure}
\FloatBarrier
\subsection{Recovered Distribution of Retinal Fluorophores}\label{subsec:meanfluor}
Having now accommodated for the dark-current, retinal fluorophores can now be recovered from SFLIM measurements using the SFLIM unmixing algorithm. Before performing the unmixing, malfunctioning pixels are masked (\cref{sec:pixelmask}), the histograms are aligned in the time axis using the shift map (\cref{sec:timealign}), and the time-bins affected by the dark current are masked. 
As previously mentioned, the unmixing algorithm is perfomed on each pixel in the SPAD image to produce a corresponding image of fluorophore concentration. The mean fluorophore concentration is then calculated and the relationship between fluorphore concentration can be established.
\\
The unmixed fluorophore maps also show no structure or variation across the image but are shown in the following plots 
\begin{figure}
    \centering
...
\end{figure}
\FloatBarrier
\subsection{Trends in Retinal Fluorophore Concentration as a Response to Acute Hypoxia}
