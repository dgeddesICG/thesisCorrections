...
In this chapter, in-vivo SFLIM images of anaesthetised rat retinas are recorded using the SFLIO device and the relative concentration retinal fluorophores are extracted using the SFLIM unmixing technique. The systemic oxygen consumption of the rat was varied in order to engineer a change in concentration of the fluorescence metabolic marker - FAD. While an image of the retina showing retinal vasculature could not be formed on the SPAD - the concentration of retinal fluorophores are considered across the whole of the retina. Trends are then established between hypoxia and the concentration of FAD and the statistical significance of these changes is estimated using paired Students t-test and found to be inconclusive and inconsistent across multiple rats. Finally, Principle Components Analysis and Spectral Angle Maps are used to determine if there is a meaningful and detectable change in the fluorescence emission spectra 
...
\end{align}
Subsequently, the minimising function, $\varphi(\mathbf{x})$ can as the sum of two seperate minimising functions over these intervals such that
\begin{align}
