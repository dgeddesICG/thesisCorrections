...
In this chapter, \textit{in-vivo} SFLIM images of anaesthetised rat retinas are recorded using the SFLIO device and the relative concentration of retinal fluorophores are extracted - demonstrating the SFLIM unmixing technique described in the previous chapter. The systemic oxygen consumption of the rat was varied, between \qty{21}{\percent}\ce{FiO2} and \qty{12}{\percent}\ce{FiO2}, to engineer a change in concentration of the fluorescence metabolic marker - FAD. A simplified model of FAD production is derived and is compared to the measured FAD concentration. While a clear image of the retina could not be formed on the SPAD - which would show key features such as the retinal vasculature - the concentrations of retinal fluorophores are considered across the whole of the retina. Trends are then established between hypoxia and the concentration of FAD and the statistical significance of these changes is estimated using paired Students t-test and found to be inconclusive and inconsistent across multiple rats. Finally, Principle Components Analysis and Spectral Angle Maps are used to determine if there is a meaningful and detectable change in the fluorescence emission spectra \textbf{of the retina}. 
\FloatBarrier
...
While the magnitude of the changes in retinal FAD distribution associated with retinal disease is not known, attempts can be made to detect the presence of FAD and assess it\textbf{s} feasibility as a diagnostic tool. Changes in FAD concentration can be induced in anaesthetised animal models by artificially restricting systemic oxygen consumption presenting as a detectable change in the SFLIM measurements. \textbf{In normoxic conditions, oxygen metabolism is typically, \qty{21}{\percent}\ce{FiO2}, however in hypoxic conditions, \qty{15}{\percent}\ce{FiO2},} the retina is starved of oxygen thus lowering the concentration of FAD.
...
\begin{figure}
    ...
    \end{annotatedFigure}
    \caption{Both (a) and (b) show examples of fluorescence images recorded \textbf{of} rat cortices after a short period of acute hypoxia, exhibiting characteristic `halos' of hyper-fluorescent FAD surrounding the vasculature. Increased FAD fluorescence is attributed to diffusion of arterial and venous oxygen into the retina to maintain metabolic function. Reproduced from Figure 3A and 3B from \citeauthor{martinez2017understanding}~\cite{martinez2017understanding}.}
    \label{fig:fadhalos}
\end{figure}
In this chapter, the SFLIO device (described in~\cref{chap:fliodevice}) was used to record \textit{in-vivo} SFLIM images of the retinas of anaesthetised rats under normoxic, hypoxic, and post-mortem conditions where the concentration of retinal FAD is recovered using the SFLIM unmixing algorithm (described in \cref{chap:tensSFLIM}). Successful detection of FAD would be signified by measuring a change in the recovered retinal FAD concentration being compatible with what is expected from changes in FAD modelled from aerobic respiration. Another impactful output could be reproducing the `halo' effect, shown in \cref{fig:fadhalos}, in the retina. Finally, from this a feasibility of quantifying retinal FAD in human retinas can be explored.
\FloatBarrier
\subsection{\textit{In-vitro} and \textit{Ex-vivo} Studies of Retinal FAD}
Escalating from the simulations presented in \cref{chap:tensSFLIM} to \textit{in-vivo} experiments represents a large, and risk-laden, conceptual jump. Two experiments were performed with limited success to mitigate these risks and gain understanding of the ability of the SFLIO device and SFLIM unmixing technique in detecting changes in FAD at similar concentrations to an \textit{in-vivo} retina. The COVID pandemic limited access to rats for the imaging experiments and due to time constraints these ``failed'' experiments could not be repeated until success. However, for the purpose of demonstrating scientific rigour, and responsible use of animal models, they are briefly described with critiques and suggestions of what could be improved for future experimentation.
\subsubsection{\textit{In-vitro} Detection of FAD in a Phantom Eye}
In the first of these experiments, a glass capillary tube was filled with a mixture of aqueous solutions of FAD, fluorescein, and rose bengal. The solution of FAD was made up to a concentration of 10 times larger than what was estimated to be in the retina (this is discussed in the next section). If FAD could be detected, this \textit{in-vitro} experiment could be repeated with successively lower FAD concentrations until FAD could no longer be distinguished from fluorescein and rose bengal to give an estimation of the sensitivity of the SFLIO device and SFLIM unmixing analysis to changes in biosimilar concentrations of retinal FAD.
An unfortunate oversight in the experimental design meant that it was only at the analysis stage that the fluorescence lifetime of Rose Bengal was confirmed to be $\approx\qtyrange{750}{800}{\ps}$~\cite{jo2005ultrafast} and poor fluorescence efficiency in the excitation range of the SFLIO device (\qtyrange{460}{467}{\nm})~\cite{rauf2009solvatochromic}. This then compromises the ability to accurately recover the concentration of each fluorophore since Rose Bengal could not be excited and its lifetime cannot be resolved with the FLIMera as shown in \cref{fig:invitroSFLIM}.
\begin{figure}[htp]
    \centering
    \begin{annotatedFigure}{\includegraphics[width=0.9\textwidth]{figures/ratUCL/InVitroSFLIMFigure.pdf}}
            \annotatedFigureText{0.02,0.93}{black}{0.3}{a)}
             \annotatedFigureText{0.02,0.32}{black}{0.3}{b)}
    \end{annotatedFigure}
    \caption{An aqueous solution comprising of sodium fluorescein, FAD, and rose bengal was decanted into a glass capillary tube - with inner diameter of \qty{1}{\mm} - and positioned at the retinal plane of a phantom eye for SFLIM imaging. SFLIM imaging was carried out using a 2 minute exposure per spectral band. In panel a) brightfield images recorded using the sCMOS detector and spectral fluorescence intensity images recorded using the FLIMera are shown. In panel b) the results from the SFLIM unmixing procedure shows that FAD has been identified as a fluorescent species in the mixture but rose bengal has not. This causes fluorescence in areas surrounding the capillary tube to be classified to fluorescein.}
    \label{fig:invitroSFLIM}
\end{figure}
The efficacy of the unmixing can be estimated by comparing the relative concentration given by the unmixing with the relative concentration calculated from the excitation spectra, quantum \textbf{yield}, and molar concentration in the solution. The molar concentration, $c$, of the dyes were \qtylist{13.28; 2.46; 12.06}{\micro\mole\per\litre}, for FAD, fluorescein, and rose bengal, respectively. Given that the quantum \textbf{yield}, $\eta$ are \qtylist{0.03;0.925;0.11}{\percent} \textbf{for} FAD, fluorescein, and rose bengal, respectively, the relative contributions, $r$, can be determined from their unity-normalised emission spectra, $S(\lambda)$:
\begin{equation}
    r_{n} = \frac{c_{n}\eta_{n}\int_{0}^{\infty}S_{n}(\lambda)d\lambda}{\sum_{n}^{N}c_{n}\eta_{n}\int_{0}^{\infty}S_{n}(\lambda)d\lambda} 
\end{equation}
The expected relative concentrations show correlation with the recovered concentrations and are shown in~\cref{tab:invitroconc}. The inability to classify rose bengal, and the relatively large concentration of fluorescein are \textbf{the} likely candidates \textbf{for} the discrepancies in relative concentration where the concentration of recovered FAD is an order of magnitude higher than what was predicted.
\\
\begin{table}[htp]
    \centering
    \begin{tabular}{|c|c|c|}
        \hline
        Fluorophore & Expected Concentration & Recovered Concentration\\
        \hline
         FAD &  \num{0.0259} & \num{0.31}\\
         Fluorescein & \num{0.9699} & \num{0.55} \\
         Rose Bengal & \num{0.042} & \num{0.0002}\\
         \hline
    \end{tabular}
    \caption{The expected concentration, or ground truth concentration, for each dye is calculated in terms of a relative contribution to the fluorescence signal - summing to unity - as are the recovered concentrations obtained using the SFLIM unmixing procedure described in \cref{chap:tensSFLIM}}
    \label{tab:invitroconc}
\end{table}
\\
...
\subsubsection{Attempted Imaging of Perfused FAD in \textit{Ex-vivo} Rabbit Models}
The second unsuccessful experiment applies the techniques in Fluorescein Angiography to measure the effective quantum yield of fluorescence in the retina to be measured by perfusing an aqueous fluorescein solution through the ophthalmic artery of a post-mortem rabbit and recording FLIM images using a \qty{500}{\nm} long-pass emission filter.
To perfuse dye into the retinal vasculature, a recently sacrificed rabbit (used for cardiac research) had its anterior carotid artery cannulated and, for convenience, the head was removed. The anterior carotid supplies blood to the ophthalmic artery through the Circle-of-Willis~\cite{vrselja2014function} and \textbf{so an exogenous dye can be perfused into the retina and imaged.}
Time-lapse FLIM and fluorescence intensity images were recorded in parallel as an aqueous solution of sodium fluorescein - with concentration of \qty{1.4}{\milli\mole\per\litre} -  was perfused through the retina, using a syringe pump, at a rate of \qty{2}{\ml\per\minute}. Unfortunately due to malfunctions of the FLIMera, there were no useable FLIM images on the three separate occasions this experiment was attempted. However, as shown in~\cref{fig:rabbitperfusion}, the sCMOS detector was able to record images at multiple time points, with a \qty{1}{\s} exposure, until the detector saturated.
\begin{figure}[htp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/ratUCL/PerfusionFig.pdf}
    \caption{A aqueous solution of sodium fluorescein of concentration \qty{1.4}{\milli\mole\per\litre} was perfused through the anterior carotid artery using a syringe pump at a rate \qty{2}{\ml\per\minute}. The frame shows a autofluorescence image where no dye is circulating. Subsequent frames show how the dye perfuses through the vasculature highlighting smaller veins and vessels. In this initial trial, fluorescence intensity images were recorded once per minute for 12 minutes until the \qty{25}{\ml} syringe was emptied of which the first 5 are shown before the detector saturates.}
    \label{fig:rabbitperfusion}
\end{figure}
Again, the COVID pandemic limited the time allowed for experimental work. This meant the perfusion experiment could not be repeated using FAD which could have provided significant insights into how much FAD contributes to autofluorescence in the retina. Additionally, this could be used to estimate how the fluorescence emission spectra and the fluorescence lifetime of FAD are affected by scattering, due to retinal vasculature, and contamination, from absorption due to blood and fluorescence from the eye's lens.
The two experiments described in this section, had they been successful, would have given high confidence in the ability of the SFLIO system to record high-quality images using both the FLIMera and sCMOS detector and also appraise the biochemical resolving power of the system and SFLIM unmxing technique. Nevertheless, the decision was made to attempt SFLIM imaging of rats with the hope of detecting and quantifying retinal FAD, rather than repeating these two experiments until they were successful. 
\FloatBarrier
...
The volumetric rate at which oxygen is consumed - better termed as the rate at which blood is deoxygenated - is metabolised, $Q_{\ce{O2}}$, is calculated from the average rate of blood flow through the ophthalmic artery, $Q_{ret}$, and the relative change in saturation of blood entering, ${S\ce{O2}}_{in}$, and exiting, ${S\ce{O2}}_{out}$, 
\begin{equation}\label{eq:O2rate}
    Q_{\ce{O2}} = Q_{ret}({S\ce{O2}}_{in}-{S\ce{O2}}_{out})H\,.
\end{equation}
The rate of blood flow through the ophthalmic artery was taken as $Q_{ret} = \qty{41.35}{\micro\litre\per\minute}$ by averaging two reported measurements in the literature~\cite{dai2013absolute,riva1985blood}. The oxygen saturation - the ratio of oxygenation to deoxygenated haemoglobin in the blood - has been reported as \qty{92.20}{\percent} and \qty{55.60}{\percent} for blood entering and exiting the ophthalmic artery~\cite{dunn2016physiology}. Using~\cref{eq:O2rate} the rate of oxygen metabolisation was estimated to be $Q_{\ce{O2}}=\qty{3.155}{\micro\litre\per\minute}$ where, $H$, H{\"u}ffners constant, has the value $H=\num{0.2085}$, and is the volumetric ratio oxygen to haemoglobin in the blood~\cite{dunn2016physiology}. Although oxygen is bound to haemoglobin in blood, this value of $Q_{\ce{O2}}$ is expressed in terms of the volume of unbound oxygen at standard temperatures and pressures - volumetric rates of oxygen will be referred to in this way, herein.
...
\end{equation}
In~\citeauthor{batey1991analysis}\cite{batey1991analysis}, the concentration is defined as \qty{39.3}{\pico\mole\per\milli\gram} by weight of the retina which is within \qty{10}{\percent} of the estimation above when converted to equivalent units - $C_{FAD} = \qty{37.64}{\pico\mole\per\milli\gram}$ obtained using the result of \cref{eq:nummoles} and taking the mass of the retina as \qty{326}{\milli\gram}~\cite{batey1991analysis,feke1989blood}. While this analysis does represent a simplified interpretation of the biological functions in the retina it is commensurate with reported measurements of flavoproteins in rabbit eyes. 
\\
The volume of FAD in the retina at hypoxia can then be estimated by assuming that the drop in maximal oxygen consumption, ${\dot{V}\ce{O2}}_{max}$, due to hypoxia will induce a decrease in the volume of retinal FAD of an equal order. This rate of maximal oxygen consumption was measured \textbf{for} sedentary and endurance trained subjects as reported in~\citeauthor{ferretti1997decrease}\cite{ferretti1997decrease} and shown in~\cref{tab:VO2max}. 
...
\end{figure}
However, in one instance the rat exhibited laboured breathing causing motion that was not just horizontal and vertical translations (see the video sequence titled ``\href{https://gla-my.sharepoint.com/:v:/g/personal/d_geddes_1_research_gla_ac_uk/EZ8927A1_dJBuev00T-clKcBANTjQIsmYPI3QbF40p4u3A?e=DPgnj6}{DGeddesPhD\_RatBreatingMotion.mp4}''). The differences in image magnification or rotation were small enough in amplitude and frequency that motion can be adequately corrected and characterised using the previously described method in~\cref{chap:motionreg}. 
...
\begin{figure}[htp]
    ...
    \caption{Montage of brightfield image sequence recorded at normoxia after the rat had previously been acutely hypoxia (\qty{12}{\percent}\ce{FiO2}) for a period. Laboured breathing in the rat caused artefacts in the sequence as show above. This is better resolved by viewing the supplementary video ``\href{https://gla-my.sharepoint.com/:v:/g/personal/d_geddes_1_research_gla_ac_uk/EZ8927A1_dJBuev00T-clKcBANTjQIsmYPI3QbF40p4u3A?e=DPgnj6}{DGeddesPhD\_RatBreatingMotion.mp4}''. This motion is correctable using the existing motion correction algorithm discussed elsewhere \cref{fig:ratbreathcorrection}}
...
\begin{figure}[htp]
    ...
    \caption{The motion due to laboured breathing does not introduce artefacts in images formed from summing frames over the entire sequence before (left) and after (right) the motion is corrected. \textbf{The pulsating motion is short, on the order of 3 frames, and thus does not significantly degrade the un-corrected image (left), making it indistinguishable from the corrected image (right) and a single elemental image (middle)}. The translational motion in the X (horizontal) and Y (vertical) directions is plotted (bottom) and shows the periodic nature of the breathing motion.}
    \label{fig:ratbreathcorrection}
\end{figure}
The amplitude of the breathing motion is small enough \textbf{such that} at motion with an amplitude of $\approx \num{10}$ pixels on an sCMOS image would manifest as $\approx \num{2}$ pixels on the FLIMera.
...
Finally, an additional pair of SLR lens with higher and lower magnifications could \textbf{have} been trialled to assess how far out-of-focus the FLIMera was despite the convallaria sample being in-focus.
\FloatBarrier
\section{Recovering FAD Concentration Using SFLIM Unmixing}
Although clear fluorescence images of the retina could not be formed on the FLIMera, the concentration of retinal FAD can be estimated over the entire FOV using the previously developed SFLIM unmixing algorithms. The SFLIM unmixing method was applied to SFLIM data with and without the application of motion correction. The relative concentrations of retinal fluorophores for each pixel on the FLIMera and the mean concentration can be calculated after masking out malfunctioning pixels with the pixel mask discussed in~\cref{sec:pixelmask}. The recovered fluorophore concentrations were then compared over the Normoxia, Hypoxia, Normoxia, and in one case~\textit{post-mortem} to establish if the detected changes fitted the predictions made in~\cref{sec:FADderivation}. The statistical significance was calculated using a paired Student t-test to establish whether a change in retinal fluorophore concentration was detected as a result of a change in the rats physiology or just due to noise. \textbf{Finally, Princip}al\textbf{ Components Analysis (PCA) was used to assess whether what was detected as changes in FAD concentration was correlated with a change in the fluorescence emission spectra or fluorescence lifetime of the retina.}
\FloatBarrier
\subsection{Correction of Anomalous After-Pulse}\label{subsec:darkfield}
In the first two of three imaging sessions the fluorescence decay was contaminated by an \textbf{anomaly} which appeared as a Gaussian-like function superimposed onto the fluorescence decay (see \cref{fig:darkfieldGauss}). \textbf{Additional} measurements were recorded with the FLIMera covered and the laser shutter closed, such that any signal recorded would originate from either thermal noise or due to this anomalous after-pulse. 
After discussions with the manufacturer of the FLIMera it was confirmed this \textbf{anomally} was due to poor calibration of the pulse-converter which converts the analogue signal produced by the laser when a pulse is emitted to a digital signal that is at logic-high when photons are to be counted. \textbf{This anomaly was concluded to be the result of a 2nd rising edge in the trigger signal resulting in an electrical signal being erroneously counted a photons.} This was corrected for the final rat imaging session and subsequent fluorescence decays did not exhibit this dark current.
\\
\begin{figure}[htp]
    \centering
...
\end{figure}
\\
If not accommodated in the unmixing process, this time-dependent \textbf{anomaly} would result in poor convergence of minimising-function, $\varphi(\mathbf{x})$:
...
\end{align}
While the \textbf{anomaly} could be characterised by fitting a Gaussian:
\begin{equation}\label{eq:gauss}
    y(x, A,\mu,\sigma,c) = A\exp(\frac{-(x-\mu)^{2}}{2\sigma^{2}})+c\,,
\end{equation}
(\cref{eq:gauss}) or $\sech^{2}$:
\begin{equation}\label{eq:sech}
    y(x,x_{0},A,\tau) = A\sech^{2}\bigg(\frac{x - x_{0}}{\tau}\bigg)+c\,,
\end{equation}
...
\end{equation}
to each pixel and then subtracting the fitted profile from the affected histogram but this could result in over-fitting in histograms with low photon counts - increasing sensitivity to noise. 
The masking approach, however, trades-off leaving insufficient photons for unmixing when too many time bins are masked and masking too few bins leaves the histogram still contaminated - biasing the recovered flourophore concentrations. A masking window of 30 pixels was selected to strike this balance which left on around \num{1000} photons used in the unmixing process after masking the dark-current and excluding the first $\approx 50$ time-bins to remove the influence of the temporal response of the SFLIO device. This process did result in more instances where the minimiser could not converge to a suitable minimum of $\varphi(\mathbf{x})$ which results in a vector of ``NaN''s being returned - which are masked out in the SFLIM unmixing process. 
\\
...
\begin{figure}[htp]
    ...
    \end{annotatedFigure}
    \caption{When using the fitted Gaussian as a mask to remove the \textbf{anomaly} there is no affect to the structure or spatial variation in the raw SPAD image (a) or the \textbf{anomaly} corrected image (b)}
    \label{fig:masked-unmaskedintensityimage}
\end{figure}
The \textbf{anomaly}-corrected images, \cref{fig:masked-unmaskedintensityimage}b, show only minimal differences to the SPAD image, \cref{fig:masked-unmaskedintensityimage}a. \textbf{This anomaly is then not the cause of the SFLIM images of rat retinas, shown throughout this chapter, having no resolvable details such as arteries and veins.}
\FloatBarrier
\subsection{Trends in Retinal Fluorophore Concentration as a Response to Acute Hypoxia}\label{subsec:meanfluor}
The concentrations of the retinal fluorophores, FAD, AGE, and A2E, can now be estimated from SFLIM measurements with the SFLIM unmixing algorithm and any trends due to changes in oxygenation state can be established. Before this analysis, the SFLIM data is processed to mask malfunctioning pixels, the fluorescence decays are aligned in the time axis, and the time-bins affected by the previously mentioned \textbf{anomaly} are also masked as described in \cref{sec:pixelmask}.
...
\end{equation}
to calculate the overlap between pairs of spectra pertaining to oxygenation states $\mathbf{a}$ and $\mathbf{b}$ with a range of \numrange{-1}{1}. High correlation between the spectra of two oxygenations states will show a $\varphi$ of \num{0} and spectra with no correlation will have a $\varphi$ near \num{1} or \num{-1}.\\
\textbf{The SAM is normally calculated as an angle, $\theta = \arccos(\varphi)$, and expressed in radians. However, arccosine function is not surjective (two values of $\varphi$ can map onto the same $\theta$) meaning that some information about the overlap between the two spectra is lost. In this analysis, the SAM is calculated using~\cref{eq:sam2} to more accurately determine the distance between oxygenation states, in PCA space, herein the value of $\varphi$ is referred to as the Spectral Angle Map or SAM.}
...
Another improvement to the unmixing process would be to adapt the existing linear-unmixing model to use a\textbf{n} auto-encoder network, or Non-negative Matrix Factorisation which can leverage large libraries of reference spectra without the increased sensitivity to noise that is often the case with simple linear unmixing models.
