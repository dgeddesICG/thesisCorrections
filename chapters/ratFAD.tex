...
Additionally, the poorer spatial resolution of the FLIMera image could be the sole reason for apparent the lack of blood vessels in the SPAD images. The $f/1.7$ field lens introduces an additional magnification factor, the relative magnification between the sCMOS detector and the FLIMera remains constant. In sCMOS images, the width of a vein was determined to be 40 pixels representing $\approx 9$ pixels on the FLIMera given the relative magnification of 4.2 between the FLIMera and the sCMOS found in \cref{chap:motionreg}. With a width 9 pixels, this represent the MTF value of 0.1 cycles per pixels the MTF is < 0.2 (\cref{fig:FLMTFResult}) meaning that any vessel would have prohibitively low contrast assuming the field lens does not introduce any aberration - widening the FWHM of the PSF. To rigorously test this hypothesis, the field could be fitted to the SFLIO device and the knife-edge test could be repeated, and the width of the PSF could be compared to reported diameters of arteries and veins in rat retinas. For example, \citeauthor{zhi2011volumetric}, reported that in mature rats the veins in the retina have diameters in the range \qtyrange{52}{55}{\um} and measurement of an artery yielded a diameter of \qty{70}{\um}.
